...
## 완료 판정 — 규율이다 (2026-08-08 이후 강제 장치 없음)
...
> **2026-08-08 정정 — 다시 아무것도 강제되지 않는다.** 순정 하네스로 전환하면서
> `autonomous-batch-gate.sh` 를 포함한 훅 배선을 전부 지웠다. 위 세 문단은 이력이다.

아래 기준은 그대로 유효하지만 **어느 모드에서도 자동으로 강제되지 않는다** —
모델이 직접 대조해야 하고, 대조하지 않으면 그냥 통과한다. 그 사실을 알고 쓴다.
...
| **Pass 3** | 백로그 §1 `PASS3_GATED` 28항목 `VERIFIED` + **독립 평가가 현재 캡처에 대해 돌았을 것** | + 캡처 세트·독립 평가·콘솔 오류 |
| **Pass 4** | **모든** Required `VERIFIED` + 시각 `ACCEPT` | + 전체 테스트·빌드·10층 증거·미커밋 0 |

> **Pass 3 은 `ACCEPT` 를 요구하지 않는다** (2026-08-02 사용자 결정, `DECISION_LOG.md:384-402`).
> `ACCEPT` 의 정의는 「판독성·스타일 평균 4.0 이상 · 2점 이하 없음」이고 그건 **VERIFIED 급 기준**인데
> Pass 3 의 바는 CONNECTED 라 두 층위가 어긋나 있었다. 그래서 Pass 3 은 **평가가 현재 그림에 대해
> 돌았는가**만 묻고, 4.0/4.0 `ACCEPT` 는 Pass 4 로 옮겼다 — 기준을 낮춘 것이 아니라 묻는 시점을
> 바로잡은 것이다. **`ACCEPT` 요구 자체는 사라지지 않는다.**
>
> 이 표는 그 결정 뒤에도 갱신되지 않아 사흘 동안 검증기와 다른 기준을 지시했다
> (2026-08-05 독립 감사 F-7 이 지적, 같은 날 정정). 모든 에이전트가 가장 먼저 읽는 파일이
> 게이트와 다른 말을 하면, 어느 쪽이 현행인지 문서만으로는 판정할 수 없다.
...
이 리듬을 강제하던 Stop hook 은 **2026-08-08에 전부 걷어냈다. 이제 순전히 규율이다** —
어느 모드에서도 아무것도 강제하지 않는다.

## 현재 걸려 있는 하네스 — **없다** (2026-08-08 순정 전환)

2026-08-04에 선별 복원했던 6개를 **전부 걷어냈다.** `.claude/settings.json` 의 `hooks`
블록을 통째로 지웠고 남은 것은 `permissions` 뿐이다 — 그건 커스텀 하네스가 아니라
Claude Code 순정 기능이다. 사용자 지시: 「순정 하네스로 진행한다」.

스크립트 21개는 `.claude/hooks/` 에 그대로 있다. **배선만 끊겼고 파일은 살아 있다.**
되살리려면 아래 이력의 배선을 `settings.json` 에 다시 넣으면 된다.

⚠ 그리고 Mac 에는 PowerShell 이 없다. 복원하더라도 `verify-topdown.ps1` 과
`unity-modal-autoclick.ps1` 에 의존하는 훅은 맥에서 **조용히 `exit 0`** 한다.

### 걷어내면서 사라진 것 — 이제 손으로 지킨다

| 잃은 장치 | 대신 지켜야 할 규칙 |
|---|---|
| `unity-worktree-guard.sh` | 에디터가 살아 있는 동안 워킹 트리를 다시 쓰는 git 명령을 쓰지 않는다. `.unity`/`.prefab`/`.mat` 을 `Write`/`Edit` 로 고치지 않는다 |
| `commit-gate.sh` | 코드를 고친 뒤 `Ascend/Run Self Tests` 를 돌리고 커밋한다 |
| `font-atlas-guard.sh` | 폰트 아틀라스를 커밋에 섞지 않는다 |
| `unity-preflight.sh` | MCP 호출 전에 Unity 생존과 모달 잠금을 직접 확인한다 |
| `autonomous-batch-gate.sh` | 자율주행 완료 판정을 모델이 직접 대조한다 |

**전부 이 저장소가 실제 실패에서 얻은 규칙이다. 장치가 없어졌다고 규칙이 없어진 게 아니다.**

<details>
<summary>이력 — 2026-08-04 선별 복원 당시의 배선</summary>
...
**이걸 막던 훅(`unity-worktree-guard.sh`)은 2026-08-04에 복원했다가 2026-08-08에 다시
걷어냈다. 지금은 걸려 있지 않다.** 모달 오토클릭(`unity-modal-autoclick.ps1`)은 배선도
없고 Mac 에는 PowerShell 도 없다.

**그러니 아래는 전적으로 손으로 지킨다. 막히지 않는다는 것이 안전하다는 뜻이 아니다.**
