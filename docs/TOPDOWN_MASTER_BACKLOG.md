...
| UP-FIX-70 | **프레임타임 하드 플로어가 뚫렸다 — 원인 미확정** — 10층 PlayMode 가 `p95 89.65 ms / 하한 16.67 ms`(11 FPS)로 처음 실패했다(2026-08-03 22:17, `2173 PASS / 1 FAIL`). 같은 규모가 직전까지 `2174 PASS / 0 FAIL` 이었고 `CheckFrameTimeHardFloor` 는 이번 diff 에 없다 — **검사가 엄해진 게 아니다.** 소요도 1023.0 → **1119.0초(+9.5%)**. 다만 런 내내 Blender 가 343MB 로 상주하고 다른 세션이 MCP 로 조립 중이었고, 이 p95 는 **마지막 600표본 창**이라 정확히 그 시간대다. 오염과 회귀를 아직 못 가렸다 | UP-TECH-07 (PRD §13.1) | 열림 — **사용자 결정으로 이 실패를 안고 커밋했다(2026-08-03).** 가르는 방법은 하나: Blender·타 세션을 멈춘 조용한 환경에서 재실행(약 19분). 통과하면 오염 확정, 재실패면 회귀이므로 `RenderBudgetProbe` 와 이번 배치의 드로우콜·오버드로우(신규 `Slab_*` 메시 92건, `RM_SoulShell_*` 9종)를 조사한다 |
