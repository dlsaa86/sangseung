...
| UP-FIX-58 | 영혼 색이 요구(밝은 적색 중심 + **검붉은** 외곽)와 달리 연어살빛~테라코타 단일 존. 원거리 35px 에서 단일 색 점으로 붕괴 | UP-VIS-02 | 열림 |
| UP-FIX-59 | 외곽 프레임이 앵커·플랜지 없는 균일폭 액자로 읽힌다. 후면 프레임 38mm 돌출은 1848mm 대비 2% 라 사선에서 분리되지 않는다 | UP-VIS-03 | 열림 |
| UP-FIX-60 | **굵기 3단이 2단으로 읽힌다** — 30mm 격벽이 14mm 돌출해 하이라이트를 얻으며 58mm 리브보다 굵어 보인다. 의도한 서열이 뒤집혀 보인다 | UP-VIS-03 | 열림 — 깊이 위계 뒤집기의 **대가**다. 릴 띠와 굵기 서열이 서로 당긴다 |
...
| UP-FIX-63 | 부유하는 「계약」 텍스트 3개 — 판도 테도 없이 흰 글자만 벽에 얹혀 있다 | UP-VIS-10 | 열림 |
| UP-FIX-64 | 그레이박스에 정체 불명의 **순수 검정** 사각형 (그레이 오버라이드 미적용) | UP-VIS-06 | 열림 |
| UP-FIX-65 | **매니페스트에 게임 상태가 없다** — 층·스핀 결과·계약 상태·위험 단계가 없어 심볼 3종 구분·계약 3요소·계기 지시값을 **판정할 수 없다** | UP-VIS-06 | 열림 (**증거 프로토콜**) |
| UP-FIX-66 | 공통축–기어박스 접합부가 어느 캡처에서도 제대로 프레이밍되지 않았다 — **판정은 통과했지만 증거는 우연히 통과했다** | UP-VIS-06 | 열림 |
