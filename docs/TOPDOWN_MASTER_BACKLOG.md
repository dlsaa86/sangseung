...
| UP-FIX-96 | **계기탑 캡션·예비줄이 판독 하한 미달.** `스핀 남음`·`층 상승` 캡션 8px(A)/9px(D)/10px(H), 예비줄 `배수 0.00배 손실 —` 11/13/15px. 자인 한글 하한 16px | UP-VIS-09 | 열림 (2026-08-04 6차 독립 평가) — **기본 선 자세 A 에서 두 큰 숫자의 라벨이 뭉개져 `5`·`0` 이 무엇의 값인지 화면만으로 알 수 없다.** 숫자는 35~47px 로 잘 읽히는데 캡션이 죽어 의미가 안 붙는다. H 에서만 겨우 읽힌다 |
...
| UP-FIX-100 | **캡처 측정기가 프러스텀 포함을 확인하지 않는다.** 매니페스트 「운행 계기탑 글리프」 표의 `G_gauge_face` 행이 `스핀숫자 152px … 탱크관 450px` 로 적혀 있으나 **그 이미지에 계기탑이 아예 없다** | UP-VIS-06, UP-TEST-10 | 열림 (2026-08-04 6차 독립 평가) — 같은 표가 `C`·`E` 는 `화각밖` 으로 정확히 판정하므로 **G 행만 투영 크기만 계산하고 화각 포함을 안 봤다.** 평가자가 이미지를 채택해 판정은 지켰지만, **측정값이 이미지와 어긋난 채로 제출됐다는 사실 자체가 증거 신뢰도 문제**다. 고치지 않으면 다음 라운드 수치를 믿을 수 없다 |
