...
| UP-FIX-93 | **`요구 0 0%` 자간 압착** — A·D·G 에서 `요구`/`0`/`0%` 세 토큰의 간격이 균등해 **요구값과 달성률을 가를 수 없다.** 직전엔 `요구 0   0 %` 로 분리돼 있었다 | UP-DEVICE-06, UP-VIS-09 | 열림 (2026-08-04 5차 독립 평가, **회귀**) — `UP-FIX-88` 수정의 부작용이다. **F 의 프레임 절단을 얻는 대신 A·D 에서 두 값의 시각 분리를 잃었다.** 판독성 「전력↔요구 두 줄 구분」이 4 → **3 으로 하락**. 서식 문제라 되돌리기 싸다 |
| UP-FIX-94 | **신설 포즈 `G_gauge_face` 가 `VISUAL_SPEC` §12 수치 대역을 셋 위반한다** — p99 **.9033**(대역 .25~.36, 2.5배) · &gt;0.50 **1.59%**(≤0.5%, 3.2배) · &lt;0.02 **34.33%**(≤32%). 이 저장소가 기록한 것 중 가장 밖이다 | UP-VIS-01, UP-VIS-02 | 열림 (2026-08-04 5차 독립 평가, **회귀**) — 원인은 `UP-FIX-90`(방 안 최고 백색이 계기 텍스트)과 **같은 것**이고 이 포즈에서 최대치가 됐다. `gray/G` 에서 글자가 **순백으로 타고 헤일로가 붙는다.**<br>⚠ A/C/D 집계는 6/7 로 통과한다 — **총량 지표가 이 결함을 못 잡는다**는 `UP-FIX-90` 의 지적이 포즈 단위에서 재확인됐다. 스타일 「§12 수치 대역」이 4 → **3 으로 하락** |
| UP-FIX-89 | **정보 위계가 뒤집혀 있다.** B 기준 과수확 35px &gt; 전력 31px &gt; **실행 24px** 인데 `VISUAL_SPEC` §5 는 실행이 **1순위**다. 또한 A/D 의 초점 덩어리가 3×3 보드에서 **백색 6줄 숫자 블록**으로 이동했고, 블록 내 7개 항목이 전부 같은 크기·같은 백색이라 §5 「한 화면에 모든 숫자를 동시에 강조하지 않는다」에 걸린다 | UP-VIS-01, UP-VIS-09 | 열림 (2026-08-04 4차 독립 평가 + 구현자 자기 신고) — **크기로 풀 수 없다.** 「실행」을 더 키우면 「글자가 레버 실물보다 무겁다」는 직전 지적과 충돌한다. **위계를 맞추려면 레버 형상 쪽을 키우거나, 숫자 블록 내부에 강약을 넣어야 한다** — 값이 아니라 배치·형상 결정 |
| UP-FIX-90 | **방 안 최고 백색이 계기 텍스트가 됐다.** 레퍼런스는 「붉은색만 강조색 / 계기 숫자도 붉다」인데 반대가 됐다 | UP-VIS-01, UP-VIS-02 | 열림 (2026-08-04 4차 독립 평가, 회귀) — ⚠ **총량 지표로는 안 잡힌다.** 평가자 註: 「g/r 총량 지표(.8760, 대역 안)는 면적이 작아 이 문제를 잡지 못한다」. 즉 §12 수치 대역을 통과해도 이 결함은 남는다 — **대역 검사에 구멍이 있다는 증거이기도 하다** |
