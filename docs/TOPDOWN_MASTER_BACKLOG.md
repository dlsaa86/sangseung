...
> **⚠ UP-FIX-80 과 UP-FIX-85 는 같은 증상의 서로 다른 원인이다.**
> 평가자는 「'실행' 표찰이 글자로 안 읽힌다」 하나를 지적했고, 나는 그것을 **색수차 탓으로만**
> 보고 색수차를 껐다. 화소 검증까지 통과했다(R→B 시프트 (−4,+1) → (0,0), NCC 0.996).
> **그런데 글자는 여전히 없다** — 애초에 렌더된 적이 없었기 때문이다.
>
> 한 증상에 원인이 둘이면, 하나를 고치고 그 검증이 통과하는 것이
> **다른 하나가 없다는 증거가 되지 않는다.** 색수차 제거는 실제로 옳았고 측정도 옳았지만,
> 그 성공이 나머지 절반을 가렸다. 씬 소유자가 메시 정점 수를 찍어 보지 않았으면
> 「고쳤다」로 끝났을 것이다.
