...
| UP-FIX-88 | **계기판이 B·F 포즈에서 우측 프레임 밖으로 절단된다.** `층`, `0 %`, `무게 0/0` 의 끝자리가 잘린다. F 는 `정상` 첫 글자가 레버 팔에 가린다 | UP-VIS-06, UP-DEVICE-06 | 열림 (2026-08-04 4차 독립 평가, 회귀) — **`UP-FIX-87` 을 고치면서 새로 생겼다.** 평가자 註: 「X 에서는 패널이 비어 있어 잘릴 내용이 없었다 — 기능적으로는 개선이지만 이 고정 세트에서 새로 생긴 결함이라 회귀로 적는다」. 고칠 대상은 글자 크기가 아니라 **판독면 폭 또는 포즈 프레이밍** |
