...
| UP-FIX-82 | 🚨 **심볼 3종이 실루엣으로 갈리지 않는다.** 9칸이 전부 「회색 원반 위 붉은 각진 덩어리 하나」이고 **회색조로 바꾸면 구분이 완전히 사라진다.** `VISUAL_SPEC` §4 는 색 외 **최소 세 항목**이 달라야 한다고 요구한다 | UP-SYMBOL-*, UP-VIS-01 | 열림 — **판독성 평균이 2.6 대에 묶여 있는 진짜 원인.** 심볼(#3)과 전력 판독(#4)이 2점인 한 **평균 4.0 은 산술적으로 불가능하다.** 셸을 아무리 고쳐도 2.6 대에서 멈춘다. 조명·그레이딩을 몇 번 더 돌려도 절대 안 올라간다 — 9칸이 같은 모양인 한 올라갈 수가 없다. **셸 작업 범위 밖 · `SYMBOL_DESIGN_SPEC` 영역 · 다음 스토리 1순위** |
| UP-FIX-83 | **캡처 세트에 3물체(실행 레버·과수확 레버·전력 탱크)가 다 들어오는 지점이 없다.** 4장 어디에서도 **실행 레버 본체가 안 보이고 표찰만 있다.** `D-20260730-08` 의 3물체 분리를 캡처로 **판정할 수 없다** | UP-VIS-06 | 열림 — 평가자가 「자료 부족」으로 판정을 보류한 항목. 기준선·신규 빌드 공통 |
...
| UP-FIX-86 | **표찰 글자가 너무 작다.** 표찰 판 147×247 px 위에 글자가 **19×16 px** — 판 면적의 **0.8%**. `ExecutionLabel` fontSize 6.20 × lossyScale 0.0350 = **0.217** vs `OverharvestLabel` 10.00 × 0.0500 = **0.500** (2.3배 차). 평가자의 「글자로 안 읽힌다」의 **실제 원인**이고 금지 #20(심볼의 작은 글자)에 걸린다 | UP-VIS-01, UP-VIS-10 | 열림 — 크기 변경은 이번 라운드 금지 범위라 손대지 않았다 |
| UP-FIX-87 | 🚨 **계기판 5개와 계약 표찰 3개가 네 포즈 전부에서 0픽셀이다.** 화물칸 안쪽을 향하고 있지 않고(단면 재질이라 **벽 뒤에서만 보인다**) `PanelBack`/`ContractPanel` 에 가려 있다. **폰트를 고쳐도 플레이어는 못 본다** | UP-VIS-01, UP-DEVICE-06 | 열림 — **평가자가 항목 #4(현재 전력·위험 이해)에 2점을 준 이유가 이것일 가능성이 높다.** 「눈금·요구치·임계점·수치가 어디에도 없다」는 지적과 정확히 맞는다. 배치·회전 문제라 구조 변경이며 `visual-verify` §6 의 「값이 아니라 배치」에 해당 |
