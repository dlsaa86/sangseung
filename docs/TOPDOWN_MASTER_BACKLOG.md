...
## 0.35 「남은 문제」를 적는 규칙 — 실측을 함께 적는다

> **2026-08-02 신설.** 하루에 낡은 기록 **다섯 건**을 잡고 나서 만든 규칙이다.

`⑤`(과적 단위) · `⑨`(대사 필드) · `UP-POWER-07`(소비처) · `UP-TEST-06`(릴리스 가드) ·
`UP-RISK-08`(사이렌) — 다섯 다 **이미 해결된 것을 미해결로 적고 있었다.** 그중 둘은
「합치면 된다」·「가드가 하나도 없다」처럼 **시키는 대로 하면 밸런스를 깨거나 이미 있는
것을 한 벌 더 만들게 되는** 문장이었다.

원인은 하나다. **「무엇이 없다」를 적을 때 그것을 어떻게 셌는지가 같이 안 적혀 있다.**
그래서 다음 사람이 다시 셀 수 없고, 고쳐진 뒤에도 문장만 남는다.

**규칙**: `남은 문제`에 「없다 · 0곳 · 미구현 · 안 된다」를 쓸 때는 셋을 함께 적는다.

1. **어떻게 셌는가** — 파일과 줄 번호, 또는 실행한 명령
2. **센 수** — 「소비처 0곳(`Scripts/Data/Profiles`·테스트 제외)」처럼 제외 조건까지
3. **언제** — 날짜

없으면 그 문장은 **주장이지 실측이 아니다.** 다음 감사자는 재측정하고, 사실이 아니면
취소선으로 남기고 정정한다 — 지우지 않는다. 지우면 왜 오래 묶여 있었는지가 사라진다.

**「무엇이 없다」와 「무엇을 고쳐야 한다」를 같은 문장에 쓰지 않는다.** 전자는 실측이고
...
> **죽은 경로**(`_contractLabel: {fileID: 0}`)에 배선했는데 **컴파일도 되고 단정 2건도
> 통과했다.** 값 층위 테스트는 「함수가 옳은 문자열을 만드는가」만 증명하고 「그 문자열이
> 화면에 닿는가」는 증명하지 못한다 — §0.4 가 코드와 **도달 경로**를 따로 세는 이유다.
> 자가 복구(`FindAnyObjectByType`)되는 필드와 그냥 죽는 필드를 반드시 갈라서 볼 것.
...
- 의존: 없음
- 남은 문제: **기준 PC 불일치는 해소됐다 (2026-08-02, 커밋 `d49922d`).** 사용자가 `PD-16` 에서 **측정 기기(Ryzen 5 5600X / RTX 3070 / D3D12 / Win11)를 기준으로 확정**했고 `DECISION_LOG` 에 올라가 있다. `TECH_SPEC.md` §13 을 그쪽에 맞췄으므로 에셋과 명세가 이제 같은 값이다 — 「승인한 항목이 없다」는 더 이상 참이 아니다. **남은 실질 차단은 하나뿐이다**: 배선이 끊겨도 실패하는 단정이 없다. 렌더 프로브는 보고 줄만 붙인다
...
- 증거: `Logs/editmode_tests.txt`
- 의존: UP-PLAT-03
- 남은 문제: **임포트 규칙이 생겼다 (2026-08-02). 「0건」은 더 이상 사실이 아니다.** `.preset` 파일을 만들지 **않았다** — 직렬화 에셋이라 이 저장소에서 조용히 깨지는 부류다. 대신 `Assets/Editor/AscendImportRules.cs`(`AssetPostprocessor`)가 경로로 카테고리를 판정하고 규칙 값을 `VisualQualityProfile._textureImportRules`·`AudioMixProfile._audioImportRules` 에서 **읽는다**(하드코딩 아님). 오디오 세 갈래가 **서로 다른 값**을 갖는다 — ShortEffect `DecompressOnLoad`/ADPCM/1.00/모노강제 · Loop `CompressedInMemory`/Vorbis/0.50 · Voice `Streaming`/Vorbis/0.70. 이름만 셋이고 값이 같아지는 회귀는 「세 갈래의 적재·압축이 서로 다르다」가 잡고, PRD §13.4 「무압축 원본을 런타임에 직접 쓰지 않는다」는 「PCM 이 어느 갈래의 기본값도 아니다」가 단정한다. ~~**그래도 CONNECTED 로 올리지 않는다** — 관할 루트 아래 **텍스처 0개·오디오 0개**라 `OnPreprocessTexture`/`OnPreprocessAudio` 가 **한 번도 실행된 적이 없다.**~~ → **텍스처 쪽이 0이 아니다 (2026-08-02 실측 정정).** 디스크를 세면 관할 루트 아래 PNG 가 **18개**다 — `Art/Textures/Generated` 14 · `Art/Textures` 4. `Logs/import_rules.txt`(08:48:56)는 「텍스처 6개」로 적고 있는데 **그 보고서 자체가 낡았다**(그 뒤 절차적 텍스처가 늘었다). 즉 「한 번도 실행된 적이 없다」는 더 이상 참이 아니고, 다만 **몇 개에 실제로 적용됐는지는 보고서를 다시 돌려야 안다** — 규칙이 `importSettingsMissing`(첫 임포트)에만 걸리므로 그 전에 들어온 에셋은 재임포트가 필요하다. **오디오는 여전히 0개**라 `OnPreprocessAudio` 는 실행된 적이 없다. **다음**: `Ascend/Report Import Rules` 를 다시 돌려 개수를 갱신하고, 텍스처 18개 중 규칙이 실제로 적용된 수를 센다. 그 수가 0이 아니면 텍스처 절반은 「적용됐다」로 넘어간다. 「규칙이 있다」와 「적용됐다」는 다르고, 그 구분을 흐리는 것이 이 저장소가 반복해서 당한 실패다. `Ascend/Report Import Rules` 가 `Logs/import_rules.txt` 에 **대상 개수를 함께** 적어 이 구분을 강제한다. **남은 것 둘**: ① 실제 텍스처·오디오 에셋이 들어오면 그때 적용이 관측된다 ② 빌드 리포트의 상위 용량 기록(PRD §13.4 마지막)은 미착수. **그리고 감사가 찾은 결함**: `VisualQualityProfile` High 의 `_shadowDistance: 30` 이 `PC_RPAsset.asset:57` 의 `m_ShadowDistance: 50` 과 **어긋난다** — 성능 리포트가 거짓 조건을 인용한다(§5.1 `UP-VIS-11`)
...
- 증거: `Captures/TenFloor/manifest.txt`
- 의존: UP-POWER-01
- 남은 문제: 임계점 눈금에 숫자 라벨이 없다 (**4세션째 열려 있음** — 계기판 배치 결정 필요)
- **▶ 2026-08-04 (v9) 두 번째 탱크가 생겼다 — `ReferenceRoom/AscentColumn/PowerTank`.**
  실행 레버 **바로 위**(월드 X 0.328…1.188 · Y 2.194…2.716)에 세운 세로 채움 관이다.
  `MASTER_PRD` §7 / `MACHINE_SPEC` §4.4 의 「레버 주변에 현재 전력·유지 배수·추가 스핀 손실
  조건을 큰 단위로 표시」가 여섯 라운드 미구현이었고 사용자 지시로 구현했다.
  회색조 실측 채움 0/17/36/57/85% (0/50/100/170/240%). 눈금 6개(50·100·130·170·220·300),
  100%·220% 는 붉은 띠. 구동은 `Scripts/View/AscentColumnView.cs`, 조립은
  `Assets/Editor/AscentColumnBuilder.cs`(멱등 · 2회 실행 바이트 동일).
  ⚠ **눈금 숫자 라벨은 여기에도 없다** — 글자를 늘리면 `UP-FIX-90` 이 악화되므로
  길이와 붉은색으로만 구분했다. 이 항목의 3세션째 미해결은 그대로 열려 있다.
  ⚠ 기존 `InstrumentPanelView` 의 전력 막대는 **발광이 죽어 있다** — `RM_Steel` 에
  `_EMISSION` 키워드가 꺼져 있어 `MaterialPropertyBlock` 의 `_EmissionColor` 가 무효다.
  계기탑은 전용 재질 `RM_GaugeFill` 로 피해 갔고, 옛 막대는 **고치지 않고 기록한다.**
- **▶ 신규 — 「지금 전력이면 몇 층 오르는가」** (사용자 지시 2026-08-04).
  `AscentDrum` 이 표시한다. 값은 `RunSession.PreviewFloorsGained()` 하나만 쓰고,
  그것이 실제 판정과 **같은** `FloorSession.PreviewAscent()` + `ClampAscent()` 를 부른다 —
  화면과 실제 상승이 갈라질 수 없다. 8개 전력값 대조표가
  `Captures/symbols_v4_20260804/manifest.txt` 에 있다.
  ⚠ **1층에서는 이 값이 거의 +1 고정이다** — 커리큘럼 보호(`ClampAscent`)가 다층 점프를
  자르기 때문이다(원시 3·4 → 1). 표시는 정직하지만 「정확히 정차하는 재미」가 실제로
  성립하는 층이 어디인지는 **아직 조사되지 않았다.**
...
- 남은 문제: **단일 원본은 진짜다 — 그러나 증거가 그것을 담고 있지 않다.** 코드 구조는 확인됐다: `AccidentRecorder.cs:65-67` 이 `FloorRecord` 하나를 만들고, `GameHudView.cs:220-227` 과 `PaperTapePrinterView.cs:154-183` 이 **씬에서 같은 fileID 998528534** 를 가리킨다 — 서로 다른 기록기를 볼 여지가 없다. **그런데** 증거로 건 `Logs/editmode_tests.txt` 226줄·10스위트·194 PASS 에 `AccidentRecorder`·`FloorRecord`·HUD·프린터 스위트가 **하나도 없다.** 「인게임 출력과 디버그가 같은 값을 말한다」를 확인하는 단정은 저장소에 0건이다. 게다가 프린터 쪽 유일한 디스크 관측(`Logs/waveb_runtime.txt`)의 「인쇄된 줄 2」는 `PaperTapePrinterView.cs:112-113` 의 **머리글**이지 `FeedRecord` 산물이 아니다 — 테이프는 층 기록을 한 줄도 찍은 적이 없다. `DEAD_IMPLEMENTATION_AUDIT` §7 의 「판정만 남았다」가 틀렸다. 남은 것은 판정이 아니라 **증거**다.
  ...
  > **왜 한 줄도 안 찍혔는지 원인을 찾았다 (2026-08-02).** 씬의 유일한 `PaperTapePrinterView` 인스턴스에서 `_tape` · `_tapeText` · `_printHead` 가 **셋 다 `{fileID: 0}`** 이다(`docs/runtime/DEAD_SCENE_WIRING.md`). `_recorder` 는 물려 있으므로 `PollNewRecords` → `FeedRecord` 는 돌고 줄이 큐에 쌓이지만, `Redraw()` 가 `:213` 에서 `_tapeText == null` 로 즉시 반환해 **그릴 대상이 없다.** `UP-REC-04` 가 증거로 삼은 「인쇄된 줄 2」는 `_printed` 카운터인데 그 증가는 `Redraw()` **앞**에서 일어나므로 화면과 무관하다. **두 항목이 같은 세 필드에 걸려 있었고 서로를 몰랐다.** 이 항목의 남은 일 ①(「같은 값을 말한다」 단정 0건)은 코드로 가능하지만, ②(테이프가 실제로 찍힌다)는 **씬 배선이 먼저다**.

  > **①을 닫았다 (2026-08-02 14:32 · `Logs/editmode_tests.txt` 363·364번째 줄 · 451 PASS / 0 FAIL).** 단정 2건을 `WiringDiagnosticsTests` 에 넣었다. ⓐ **「테이프가 기록의 값을 그대로 쓴다」** — 실제 런에서 `FloorRecord` 를 만들어 `FeedRecord` 에 넣고, 쌓인 줄이 그 기록의 **바로 그 숫자**(층·요구 전력·최종 전력·무게/허용)를 담는지 본다. 재계산하면 어긋나서 실패한다. ⓑ **「그릴 대상이 없으면 줄이 쌓여도 그려지지 않는다」** — `_tapeText` 없이 `Redraw()` 를 불러 `PrintedLines` 가 **0으로 남는 것**을 고정한다. 이게 씬의 현재 상태이고, 그래서 카운터만으로는 아무것도 증명되지 않는다는 것을 테스트가 붙잡는다. `FeedRecord`·`_pending` 이 private 이라 반사를 썼다 — 공개 API 로 부르려면 씬 배선이 필요한데 **그 배선이 바로 지금 비어 있는 것**이라 검사가 성립하지 않는다. 테스트용 공개 API 를 늘리는 것보다 낫다고 판단했다. **②는 그대로 씬 오너 대기**(`docs/runtime/DEAD_SCENE_WIRING.md` 의 `_tape`·`_tapeText`·`_printHead`). 출처 `PRD §10.3` 도 동결 PRD 에 없다
...
- 증거: `Logs/render_budget.txt`
- 의존: UP-TECH-04
- 남은 문제: **처음으로 기준 해상도에서 쟀다 (2026-08-01).** `Logs/loaded_critical_perf.txt` 머리말이 이제 「해상도 **1920×1080** / vSync 0 / targetFrameRate −1」이다 — 이전 측정은 전부 816×714(화소 28%)였고 `TECH_SPEC.md` §13 기준 밖이었다. 게임 뷰를 캡처·측정 전에 고정하게 만든 것이 이 문제까지 같이 풀었다. **파티클 추가 비용도 쟀다** — GC Alloc 이 10,760 → 10,807(**+47 B/프레임, +0.44%**), 9,128 → 9,173(**+45 B, +0.49%**). 파티클 5계통(최대 동시 274)이 프레임당 50 B 미만이다. 이는 「8,805 B 바닥은 게임 상태와 무관하다」는 앞선 분해와 일치한다. **남은 것**: ~~① 드로우콜·SetPass·오버드로우는 이 프로브가 안 잰다 — `render_budget.txt` 쪽 프로브를 1920 에서 다시 돌려야 한다~~ → **①이 대부분 해소됐다 (2026-08-02 14:01 실측).** `Logs/render_budget.txt` 가 그 시각에 재생성됐고 머리말이 「해상도 1920×1080 / vSync 0 / targetFrameRate −1」이다. 담긴 값 — 드로우콜 중앙 166 / 95% 180 / 최악 636 · SetPass 중앙 69 / 95% 78 / 최악 173 · 삼각형 중앙 13987 / 95% 18047 / 최악 60934 (전부 유효 600/600 표본). **오버드로우만 여전히 없다** — 셋 중 둘이 측정됐다. ② `PD-15`(예산 값)가 미결이라 **넘었는지 판정할 기준이 여전히 없다**. 지금은 숫자를 기록할 뿐 통과/실패를 말할 수 없다.

  **②의 기준 하드웨어 부분은 해소됐다 (2026-08-02 `d49922d`) — 아래는 그 이전 맥락이다.** `render_budget.txt` 자신이 「기준 하드웨어가 확정되지 않았으므로(`ASSUMPTION_LOG` A-20260730-01) 이 수치로 성능 완료를 선언하지 않는다」고 적는다. `UP-PLAT-04` 가 막힌 이유도 `TargetHardwareProfile.asset` 이 개발 기기(Ryzen 5 5600X)를 담고 있고 `TECH_SPEC §13` 은 Ryzen 7 5700 을 지정한다는 것이다. **기준 PC 를 한 번 정하면 두 항목이 함께 풀린다** — 이 프로파일이 「목표 90 FPS / 하한 60 / 1920×1080 / vSync 꺼짐」을 이미 들고 있고 측정 조건이 그것과 같다고 파일이 확인해 준다
...
**④ 도 충족 (2026-08-02)** — `FloorCurriculumProfile` 이 10층의 요구 전력·스핀 수·저항 배율을 담고 `RunSessionBehaviour` → `TenFloorSource` → `RunSession` 으로 흐른다. **이 프로파일만 모양이 다르다: 대체가 아니라 덮어쓰기다.** 미뤄 뒀던 이유가 「`_tenFloors` 의 각 숫자에 근거가 주석으로 붙어 있어 인스펙터로 옮기면 끊긴다」였는데, 해결은 **옮기지 않는 것**이었다 — `_tenFloors` 는 근거와 함께 코드 프리셋으로 그대로 남고 에셋은 그 위에 얹힌다. 기본값이 **빈 배열**이라 에셋이 없으면 곡선이 한 자리도 안 바뀐다. `Reset()` 이 현재 곡선을 읽어 채우므로 편집자는 빈 칸이 아니라 현재 값을 보고 시작한다. **부분 배열은 부분 적용되지 않는다** — 길이가 10이 아니면 그 축을 통째로 무시한다. 절반만 맞는 배열을 절반 적용하면 어느 층이 데이터에서 왔는지 아무도 말할 수 없고, 그게 이 항목이 막으려는 바로 그 상태다. 칸 하나가 0이면 그 층만 프리셋으로 남는다(`FloorPlan` 이 0을 「명시 안 함」으로 읽는 규약을 그대로 지킨다). **층별 심볼 풀·계약 선택지·빌드 보상 여부는 넣지 않았다** — 커리큘럼의 구조이지 밸런스 다이얼이 아니다. 3층에서 계약 선택지를 없애면 난이도가 바뀌는 게 아니라 4층이 가르칠 것을 잃는다.

**② 도 충족 (2026-08-02)** — `ContractProfile` 8종(계약 2 × 출현·보상·패턴 가산·대가)이 `RunSessionBehaviour` → `TenFloorSource.For` 로 흐른다. 계약은 `FloorPlan.ContractChoices` 안에 **값 복사본**으로 들어 있고 그 배열은 `_tenFloors` 정적 초기화 때 한 번 만들어지므로, 층을 읽는 순간 갈아 끼운다. **공유 정적 배열을 제자리에서 고치지 않는다** — 한 번 덮어쓰면 그 뒤의 모든 런이 오염된다. 덮어쓸 것이 있을 때만 새 배열을 만들고, 폴백이면 원본을 그대로 통과시켜 **할당이 0**이다. 그래서 스냅샷에 `Overrides` 를 따로 뒀다: 「값이 같으니 덮어써도 결과가 같다」와 「아예 안 건드린다」는 다르다. `Validate` 는 범위가 아니라 **거래가 성립하는가**를 본다 — 출현만 올리고 아무것도 안 주는 계약, 잔류 대가가 1 미만이라 계약이 오히려 덜 아프게 만드는 값. `Target`·`Label` 은 넣지 않았다: 어느 저항체를 대상으로 하는가는 계약의 정체지 다이얼이 아니고, 바꾸면 4층과 6층이 가르치는 것이 뒤바뀐다.

**다음**: 남은 미충족은 **⑫ 하나**이고 ⑨ 는 씬 오너 대기다. ⑫ 는 `PresentationProfile` 에 표면 색·발광 칸이 **이미 있으나 소비처를 세어 보지 않았다** — ⑪ 과 같은 함정일 수 있으니 만들기 전에 셀 것. 다만 머티리얼을 건드려야 해서 **씬 소유자가 필요하고**, 2026-08-02 시점에는 다른 에이전트가 조명·재질을 재설계 중이라 정면 충돌한다. 그리고 여전히 머티리얼을 건드려야 해서 **씬 소유자가 필요하다**. **②④ 는 `FloorPlan._tenFloors` 에 있는데 그 배열은 각 숫자의 근거가 주석으로 붙어 있다** — 「2층 저항 배율 1.6 은 기본 밀도에서 직선이 스핀당 0.08회라 5스핀이면 기대 0.4회, 즉 셋 중 둘이 직선을 가르치는 층에서 직선을 못 본다」 같은 것. 인스펙터로 옮기면 그 근거가 끊긴다. 값만 옮기지 말고 근거를 어디에 둘지 먼저 정할 것
...
- 분류: Required · 출처: PRD §9 (`MASTER_PRD.md:179` — 「Strain: 과적 또는 잔류 저항 증가」). 직전 표기 `§8.1` 은 오기였다 (2026-08-02 정정)
- 상태: CONNECTED · 패스: P4
...
| UP-APV-14 | 기준 하드웨어 프로파일 확정 | **확정됨 (2026-08-02 `d49922d`).** 사용자가 측정 기기 Ryzen 5 5600X / RTX 3070 을 기준으로 골랐고 `TECH_SPEC` §13 을 맞췄다. 이전에 근거로 인용하던 `A-20260731-01` 에는 그 문자열이 **없었다** — 근거는 이제 `DECISION_LOG` 의 `PD-16` 항목이다 | PRD §13.1 |
...
### 5.0.1 — 9실 캐비닛 리블록아웃 2차 독립 판정 (2026-08-03)

전문은 `docs/runtime/VISUAL_VERDICT.md` 2026-08-03 절. 두 평가자가 독립적으로 REJECT.
**닫힌 축 넷**(세로 릴 띠 · 계층 · 동력 사슬 · 레버 결합)도 함께 확인됐다.

| ID | 결함 | 관련 | 상태 |
|---|---|---|---|
| UP-FIX-53 | 현창 9칸에 유채색 광원 0개 | UP-VIS-02 | **✅ 닫힘** — `SaveMesh` 가 `CopySerialized` 로 갱신해 **첫 빌드의 형상을 계속 그리고 있었다.** 채널 직접 덮어쓰기로 교체 |
| UP-FIX-67 | **레버 앞에 서면 결과판이 사라진다** — `04_lever_join` 9/9 창 불투명, 붉은 0.02%. `visual-criteria.md` `B-5 #15` 금지 항목. **레버를 당겨 판을 해결하는 게임인데 레버 앞이 판이 안 보이는 자리** | UP-VIS-09, UP-DEVICE-09 | **✅ 원인 확정 + 1차 조치.** 유리가 아니라 **기하 비네팅**이었다(통제 실험: 유리 제거 0.03% 무관 · 도어 0.21% · 링 0.32% · 둘 다 1.04%). 보어를 앞으로 30mm 벌리고 도어 조리개를 링 보어와 같게 → **0.02% → 0.11%**. 아직 정면(0.45%)의 1/4 이라 열어 둔다 |
| UP-FIX-68 | 전력 표시기가 방에서 가장 밝은 빈 사각형 | UP-DEVICE-06 | **✅ 닫힘** — 재질 저술은 옳았고 **에셋에 남은 발광값**(0.099, 0.083, 0.038)이 원인. `Mat()` 이 발광을 「켜는 쪽만」 써서 끈 적이 없었다. 항상 다시 쓰게 고침 |
| UP-FIX-58 | 영혼 색이 요구(밝은 적색 중심 + **검붉은** 외곽)와 달리 연어살빛~테라코타 단일 존. 원거리 35px 에서 단일 색 점으로 붕괴 | UP-VIS-02 | **부분 조치 — 진단이 뒤집혔다.** 껍질을 어둡게(base 0.105/0.011/0.007 · 발광 x0.085) 해 코어:껍질 대비를 4.8배 -> 14배로 벌렸다. 독립 평가는 「영혼이 15px 라 껍질이 담길 자리가 없다」고 했으나 **실측이 반증했다** — `03_player_eye` 껍질 45.4px · 코어 24.6px · 유리 76.7px. 평가자의 15px 는 붉은 화소 비율에서 역산한 값이라 **코어의 밝은 일부만** 센 것이다. 크기가 아니라 **배경**이 문제였다(챔버 후면이 완전한 검정이라 어두운 껍질이 검정 위 검정). 후면을 분리해 `ChamberGlow` 로 희미하게 발광. 재평가 대기 |
| UP-FIX-59 | 외곽 프레임이 앵커·플랜지 없는 균일폭 액자로 읽힌다. 후면 프레임 38mm 돌출은 1848mm 대비 2% 라 사선에서 분리되지 않는다 | UP-VIS-03 | 열림 — 다음 배치. 평가자 지시: 벽 앵커 러그 6개(120x90mm, 프레임 밖 30mm) + 기초 채널(1.868x0.14x0.09m) + 거싯 3개. **굵기가 아니라 접속부가 하중을 말한다** |
| UP-FIX-60 | **굵기 3단이 2단으로 읽힌다** — 30mm 격벽이 14mm 돌출해 하이라이트를 얻으며 58mm 리브보다 굵어 보인다. 의도한 서열이 뒤집혀 보인다 | UP-VIS-03 | **해소 (원인 축이 틀렸었다).** 폭을 58->68mm 올린 것은 **원인이 아니었다.** 399px/m 실측: 폭은 리브 26px > 격벽 13px 인데 시각 무게는 격벽 > 외곽 >= 리브. 14mm 돌출한 13px 부재는 하이라이트+그림자 **둘**을 얻고 7mm 돌출한 26px 부재는 아무것도 못 얻는다. **돌출 축으로 고쳤다** — 격벽 +14mm -> **-8mm 음각**, 리브에 28x18mm 캡을 **행마다 6토막**. 가로 연속성은 그림자선으로 남아 릴 띠 방어(무중단비 26.8%)가 살아 있다. 불변식 4개 추가(4c~4f). 부수 효과로 VISUAL_SPEC 3장의 「열 = 결과판 한 열」 대응이 형태로 복구됐다 |
| UP-FIX-61 | 상태 탭에서 챔버로 내려가는 부재가 없어 「레버 → 9개 잠금」의 마지막 링크가 화면에 없다 | UP-DEVICE-10 | 열림 |
| UP-FIX-62 | 바닥·천장이 무텍스처 회백색 프리미티브다. 벽은 조밀한데 그 둘만 매끈하고 화면에서 가장 밝은 축 | UP-VIS-01 | 열림 |
| UP-FIX-63 | 부유하는 「계약」 텍스트 3개 — 판도 테도 없이 흰 글자만 벽에 얹혀 있다 | UP-VIS-10 | **조치.** `RelocateContractPlaques()` 추가 — 명판이 비활성으로 (0.99, 1.28~1.72, +0.30) 에 있고 라벨만 (0.95, ~, -0.72) 에 떠 있었다. 명판을 켜고 라벨과 같은 자리(우벽)로 옮겼다 |
| UP-FIX-64 | 그레이박스에 정체 불명의 **순수 검정** 사각형 (그레이 오버라이드 미적용) | UP-VIS-06 | **해소.** 그레이박스 오버라이드가 `ReferenceRoom` 하위만 덮고 있었다 — 검은 사각형은 `GrayboxWorld/Car/DoorControl/Plate` 였다. 씬의 **모든** 활성 비-TMP 렌더러로 확대. 그레이 암부 0.8% / 1.0%, 순수 검정 0 |
| UP-FIX-65 | **매니페스트에 게임 상태가 없다** — 층·스핀 결과·계약 상태·위험 단계가 없어 심볼 3종 구분·계약 3요소·계기 지시값을 **판정할 수 없다** | UP-VIS-06 | **조치.** `TenFloorCaptureRig.GameState()` 추가 — 층·단계·남은 스핀·선택 계약·전력비·수확대 개방을 **각 장에** 적는다. 두 촬영 경로 양쪽에 붙였고 못 읽으면 「**확인 못 함**」이라고 적는다. 히어로 세트에는 `LockStateFacts()` — 체결 여부·진행도·핀 후퇴·공통축 각·상태 탭 y·클램프 9개 평균각 |
| UP-FIX-66 | 공통축–기어박스 접합부가 어느 캡처에서도 제대로 프레이밍되지 않았다 — **판정은 통과했지만 증거는 우연히 통과했다** | UP-VIS-06 | 열림 — `06_side78` 포즈로 진짜 측면(-78도)은 확보했으나 접합부 전용 근접 컷은 아직이다. 평가자 지적: `04_lever_join` 이 상단 레일을 화면 밖으로 잘라내 **정면보다 사슬이 덜 읽힌다** |
| UP-FIX-69 | **10층 캡처 리그의 카메라 좌표가 존재하지 않는 방의 것이다** — 구 캐빈 x[−1.20..1.20]·높이 3.20 기준이고 장치가 좌벽에 있던 시절 값이다. 그 결과 `02_device_front` 가 「결과판 9칸 중 프레임 안 **0칸** · 계기 글자줄 프레임밖 6줄」로 찍힌다. `AscendHeroCapture` 는 명세에서 끌어오는데 이 리그만 손으로 적혀 있다 | UP-VIS-06, UP-VIS-07 | 열림 (**`UP-VIS-06` 의 직접 차단 사유**) |
| UP-FIX-70 | **프레임타임 하드 플로어가 뚫렸다 — 원인 미확정** — 10층 PlayMode 가 `p95 89.65 ms / 하한 16.67 ms`(11 FPS)로 처음 실패했다(2026-08-03 22:17, `2173 PASS / 1 FAIL`). 같은 규모가 직전까지 `2174 PASS / 0 FAIL` 이었고 `CheckFrameTimeHardFloor` 는 이번 diff 에 없다 — **검사가 엄해진 게 아니다.** 소요도 1023.0 → **1119.0초(+9.5%)**. 다만 런 내내 Blender 가 343MB 로 상주하고 다른 세션이 MCP 로 조립 중이었고, 이 p95 는 **마지막 600표본 창**이라 정확히 그 시간대다. 오염과 회귀를 아직 못 가렸다 | UP-TECH-07 (PRD §13.1) | 열림 — **사용자 결정으로 이 실패를 안고 커밋했다(2026-08-03).** 가르는 방법은 하나: Blender·타 세션을 멈춘 조용한 환경에서 재실행(약 19분).<br><br>**⚠ 그림자 경로는 이미 파봤고 기각됐다 — 다시 파지 말 것.** 과거 89.03ms 회귀(본 문서 §「그리고 그 재실행이 성능 회귀를 잡았다」)와 수치가 0.7% 안에서 겹쳐 같은 원인을 의심했으나, 씬 실측 결과 **아니다**:<br>`켜진 렌더러 353 · 그림자 캐스터 137` (과거 회귀 때 279, 복구 후 118 — 지금은 복구 수준)<br>`이름이 Slab_ 인 캐스터 0건` — 이번에 추가된 신규 메시 92건은 **하나도 그림자를 드리우지 않는다.** 조립기가 0.26m 규칙을 지켰다.<br>`0.26m 규칙 위반 4건` — `Housing` 0.220m / `OilCan_B` 0.200m / `Guard` 0.221m / `Handle` 0.227m. 이번 배치와 무관한 소품이고 4개로는 279→137 규모의 비용을 못 낸다.<br>그림자 광원은 `Directional Light`(Soft) + `CabinLight`(Point·Soft) 둘.<br><br>**남은 후보**: 캐스터는 안 늘었는데 **켜진 렌더러가 353개**다. 본 패스 드로우콜과 배칭 붕괴 축을 `RenderBudgetProbe` 실측으로 봐야 한다 — 그러려면 결국 PlayMode 다.<br><br>**캡처 리드백 경로도 기각됐다(2026-08-03) — 다시 파지 말 것.** `TenFloorCaptureRig` 가 `ReadPixels`+`EncodeToPNG` 를 동기로 부르므로(1120~1127행) 유력해 보였으나, 캡처 세트는 `21:03~21:04`·오토파일럿 로그는 `22:17` 로 **다른 런**이고 오토파일럿 로그에 「캡처」로 검색되는 줄이 **0건**이다.<br><br>**대신 증거의 결함이 드러났다**: `CheckFrameTimeHardFloor` 가 `Report()` 에서 **95% 만 뽑아 적는다.** 중앙·최악이 로그에 없어 「지속형」과 「스파이크형」을 원리적으로 가를 수 없다 — 원인 축이 다른데 증거가 구분하지 못한다. 재실행 배치에서 중앙·최악·드로우콜을 함께 적도록 고칠 것.<br><br>**표본 산수(2026-08-03, 로그에서 계산)**: `RenderBudgetProbe._sampleEveryNFrames = 1` 이므로 표본 = 프레임이다. 런 전체 표본 **79,086개 / 소요 1119.0초 → 평균 약 71 FPS**. 그런데 실패한 p95 는 링 버퍼 용량인 **마지막 600표본**, 즉 전체의 **0.76%** 에서 나온다. ⚠ 즉 이것은 정상 상태 회귀가 **아니다** — 평균은 하한 60 FPS 를 넘고 **말미 구간만** 11 FPS 다. (단서: 도중 `ResetSamples()` 가 불렸다면 표본 구간이 런 전체가 아닐 수 있다. 그 경우 평균값은 무효이지만 「p95 가 말미 600프레임의 값」이라는 사실은 그대로다.) → 다음 단계는 「전반적으로 느린가」가 아니라 **「런 말미에 무엇이 도는가」** 다. 로그상 그 구간은 과수확 5단계 · 붕괴 연출 · 파티클 최대 동시 284 · 보고서 생성이 겹치는 자리다. **측정 하네스 자신의 종료 작업(문자열 조립·캡처)이 프레임타임에 섞였을 가능성**을 먼저 배제할 것 — `Time.unscaledDeltaTime` 은 그것을 구분하지 못한다.<br><br>**판단 정정(2026-08-03)**: 커밋 시점에는 「경합 오염이 유력」이라고 봤고 사용자의 「백로그로」 결정도 그 판단 위에 있었다. 89.03 vs 89.65 의 근접이 우연으로 보기 어려워 **오염 가설은 약해졌고 실제 회귀 가능성이 올라갔다.** 재실행 우선순위를 그에 맞춰 볼 것<br><br>**✅ 해소 (2026-08-03 23:38 재실행)** — **오염이 맞았다.** Blender 가 유휴일 때 같은 씬을 다시 돌린 결과:<br>`프레임타임 p95 12.95 ms` (하한 16.67 ms) — **89.65 → 12.95, 통과.** 회귀가 아니다.<br>새로 넣은 진단이 그 근거를 함께 준다 — `[드로우콜] 중앙 964 / 95% 1009 / 최악 1060 · [SetPass] 중앙 97 / 95% 114 / 최악 302 · [삼각형] 중앙 33397 / 95% 36425 / 최악 79439`. 드로우콜·삼각형이 오염 런과 사실상 동일한데(985/32981) **프레임타임만 7배 차이났다** — 씬이 무거워진 게 아니라 GPU 를 남과 나눠 쓴 것이다.<br>89.03ms 과거 회귀와 수치가 겹친 것은 **우연이었다.** 그때는 그림자 캐스터 279개가 원인이었고 지금은 137개다.<br>남길 것: 드로우콜 중앙 964 는 여전히 높다. 지금 예산을 깨지 않지만 부품이 더 늘면 먼저 무너질 축이다 |

| UP-FIX-80 | **색수차가 켜져 있다 — 금지 #17 위반.** 「실행」·「과수확」 표찰이 파랑 획과 붉은 획으로 갈라져 **글자로 안 읽힌다**(B 우상단). C 층수 표시판 적·청 분리, D 가위문 슬랫 이중선. 같은 폴더 `postOFF/` 대조로 **post 효과임이 확정**됐고 기준선에는 없던 신규 결함이다. 훼손 대상이 하필 `VISUAL_SPEC` §5 상호작용 우선순위 **1위**다 | UP-VIS-01 | 열림 — **최우선.** 값 하나. 이걸 안 끄면 이후 모든 명도 조정이 같은 위반 위에 쌓인다 (2026-08-04 `visual-critic` REJECT §E-1) |
| UP-FIX-81 | **장치 캐비닛이 자기 실루엣을 잃었다.** 캐비닛 명도 = 그늘진 벽 명도라 **3×3 을 붙들어 주는 것이 켜진 포트홀 9개뿐**이고 통관 세로 구획선은 흑 위 흑이다. 붉은 포트홀을 지우면 캐비닛이 어디서 시작하고 끝나는지 알 수 없다 | UP-VIS-01, UP-DEVICE-06 | 열림 — 문제는 방이 어두운 게 아니라 **가장 중요한 오브젝트가 배경과 같은 명도인 것**이다. 국부광으로 확실히 밝히거나 림으로 확실히 가르거나 둘 중 하나. `VISUAL_SPEC` §3 「정면에서 즉시 읽혀야」의 직접 요구 |
| UP-FIX-82 | 🚨 **심볼 3종이 실루엣으로 갈리지 않는다.** 9칸이 전부 「회색 원반 위 붉은 각진 덩어리 하나」이고 **회색조로 바꾸면 구분이 완전히 사라진다.** `VISUAL_SPEC` §4 는 색 외 **최소 세 항목**이 달라야 한다고 요구한다 | UP-SYMBOL-*, UP-VIS-01 | ✅ **조치 완료 · 독립 평가로 확인됨 (2026-08-04, `40461dd`+`d82155a`)** — 아래 진단은 **틀렸다.** 원인은 형태가 아니라 **가림**이었다.<br><br>실측: 장식 `SoulObject` 폭 **0.177** / z **0.090** vs 결과판 심볼 폭 0.145 / z 0.120 — 장식이 **더 크고 더 앞**이라 정면 가림률 100%. 평가자가 본 「회색 원반 위 붉은 각진 덩어리」는 심볼이 아니라 **장식 영혼**이었고, 심볼을 아무리 다르게 만들어도 화면에 도달할 수 없었다.<br><br>바꾼 것: ① 심볼이 영혼의 자리를 차지(`z = SoulDepthFromDoorFace`), 장식 영혼은 **렌더러만** 끔(오브젝트 18개 잔존, 삭제 0) ② 신규 `Assets/Editor/SymbolShapeFactory.cs`(533줄)가 형상 정의를 **독점** — 이전엔 `HumanScaleLayout`과 `AscendReferenceRoomRewire`가 각각 적어 두어 조립 메뉴에 따라 화면이 달라졌다 ③ `SYM_*.asset` 3종·`RM_Symbol*.mat` 2종 신규.<br><br>**회색조 분리 실측** — 정상 영혼 **1덩어리**(채움 0.751) / 흡수체 **2덩어리**(0.306) / 증식체 **3덩어리**(0.126). 덩어리 수 축은 완전 분리.<br>**독립 평가 4차: `gray심볼3종` = 4점**, 「9칸 배치가 매니페스트와 완전 일치」.<br><br>⚠ **남은 미달(구현자 자기 신고)**: 요구는 4배 분리인데 흡수↔증식 쌍이 **2.4~2.8배**에 그친다. 별 판본의 같은 쌍이 1.46~1.51배였으므로 개선은 맞다. 6갈래 별 → 육각 아가리 교체는 2차 평가의 「별은 카지노 슬롯머신 릴 어휘」 지적(`VISUAL_SPEC` §1 금지) 때문이며, 그 과정에서 두 번 헛짚었다(목구멍 덩어리 하나 → 구와 채움률 동일 / 아가리 전체 핵 재질 → 구보다 커짐).<br><br>~~열림 — 판독성 평균이 2.6 대에 묶여 있는 진짜 원인. 셸 작업 범위 밖 · `SYMBOL_DESIGN_SPEC` 영역 · 다음 스토리 1순위~~ |
| UP-FIX-83 | **캡처 세트에 3물체(실행 레버·과수확 레버·전력 탱크)가 다 들어오는 지점이 없다.** 4장 어디에서도 **실행 레버 본체가 안 보이고 표찰만 있다.** `D-20260730-08` 의 3물체 분리를 캡처로 **판정할 수 없다** | UP-VIS-06 | ⏳ **부분 조치 (2026-08-04, `40461dd`)** — 포즈 **E(`E_contract_wall`)·F** 를 캡처 세트에 추가했다. 계약 표찰이 A~D 네 포즈 전부에서 카메라 뒤였다는 사실이 `UP-FIX-87` 조사 중 드러나 함께 처리됐다.<br>다만 **3물체(실행 레버·과수확 레버·전력 탱크)가 한 화각에 다 들어오는 지점이 생겼는지는 아직 판정되지 않았다.** 4차 평가가 F 에서 「`정상` 첫 글자가 레버 팔에 가림」을 지적한 것으로 보아 F 에 레버 실물이 들어오기는 한다. **`D-20260730-08` 3물체 분리의 캡처 판정은 다음 라운드로 남는다** |
| UP-FIX-84 | **`Captures/baseline.txt` 가 이 기기에 존재하지 않는다** — 즉 **승인된 베이스라인이 없다.** 그 파일은 이미지가 아니라 `visual-verify` 가 ACCEPT 할 때만 쓰는 runId 포인터이고 `Captures/` 는 gitignore 대상이다. 「직전 승인 빌드보다 나빠지지 않았는가」를 대조할 대상이 없는 상태로 평가해 왔다 | UP-VIS-06 | 열림 (2026-08-04 씬 소유자 확인) |

| UP-FIX-85 | 🚨 **한글 라벨이 아예 렌더되지 않는다 — 폰트에 글리프가 없다.** `ExecutionLabel`('실행')·`ContractPlaqueLabel_0/1/2`('계약')이 `LiberationSans SDF` 를 쓴다. 한글 글리프가 없어 `mesh.vertexCount = 4`, 렌더러 바운즈 `(0,0,0)` — **표찰이 비어 있다.** `OverharvestLabel`('과수확')만 `NanumGothic SDF` 를 쓰고 그것만 화면에 보인다 | UP-VIS-01, UP-VIS-10 | 조치 중 (2026-08-04) — **스타일이 아니라 기능 결함이다.** `VISUAL_SPEC` §5 상호작용 계층 **1위가 「현재 사용 가능한 핵심 레버」**인데 그 레버의 표찰이 렌더되지 않았다 |

> **🔴 정정 (2026-08-04) — 위 UP-FIX-85 의 진단은 틀렸다. 씬 소유자가 화소로 반증했다.**
>
> `LiberationSans SDF` 의 **폰트별 폴백 표에 NanumGothic 이 들어 있어서** 렌더 시점엔
> 한글이 이미 그려지고 있었다. `vtx=4 / bounds=(0,0,0)` 은 **직렬화 상태**다 —
> TMP 3D 메시는 `HideFlags.HideAndDontSave`, 서브메시는 `DontSave` 라
> **저장된 씬에 `TMP SubMesh` 가 0개**다. 렌더 결과가 아니라 저장 상태를 잰 것이었다.
>
> 화소 검증: v7·v8 의 B 뷰 '실행' 발광 bbox 가 **완전 동일** (1828,846~1846,861 · 158 점등 화소).
> 폰트 교체는 판독성을 **바꾸지 않았다.** 얻은 것은 폴백 하이브리드 머티리얼 의존 제거와
> 직렬화 정합성뿐이다.
>
> **교훈은 그대로 남지만 대상이 바뀐다.** 「한 증상에 원인이 여럿」은 맞았고
> UP-FIX-80(색수차)도 실제 결함이었다. 다만 두 번째 원인은 폰트가 아니라 **UP-FIX-86·87** 이다.
> 그리고 이번엔 **내 쪽이 「직렬화 상태」를 「렌더 상태」로 오독**했다 —
> Unity 에서 「메시가 비어 있다」는 관측은 렌더 검증이 아니다.

| UP-FIX-86 | **표찰 글자가 너무 작다.** 표찰 판 147×247 px 위에 글자가 **19×16 px** — 판 면적의 **0.8%**. `ExecutionLabel` fontSize 6.20 × lossyScale 0.0350 = **0.217** vs `OverharvestLabel` 10.00 × 0.0500 = **0.500** (2.3배 차). 평가자의 「글자로 안 읽힌다」의 **실제 원인**이고 금지 #20(심볼의 작은 글자)에 걸린다 | UP-VIS-01, UP-VIS-10 | ✅ **조치 완료 · 독립 평가로 확인됨 (2026-08-04, `14bc9e9`+`3ea4a22`)** — 글자 배율 **0.3393 → 1.0000**(2.95배), A 글리프 13~18px.<br>**진짜 원인은 「글자가 작다」가 아니라 「폭을 잡아먹던 것이 화면에 없는 것들이었다」** — 렌더러가 꺼진 `PanelBack`(폭 1.70)이 경계를 지배했고 `CascadeLabel` 의 `Renderer.bounds` 가 미사용 정점 때문에 0.62 허깨비였다. 세 수정: 막대·눈금을 글자 단으로 / 꺼진 렌더러를 배율 계산에서 제외 / TMP 는 `Renderer.bounds` 대신 글리프 상자로 계측.<br>`3ea4a22` 가 「실행」 표찰을 판독면 위 벽으로 옮겨(`A-20260804-11`) 3차 1점 → 4차 **3점**.<br><br>⚠ **정보 위계는 아직 뒤집혀 있다(구현자 자기 신고)** — B 기준 과수확 35px &gt; 전력 31px &gt; **실행 24px** 인데 `VISUAL_SPEC` §5 는 실행이 1순위다. 「목표 20px 는 넘겼고 그 이상은 『글자가 레버 실물보다 무겁다』는 직전 지적과 충돌하므로 더 키우지 않았다. **위계를 맞추려면 크기가 아니라 레버 형상 쪽을 키워야 한다**」 → `UP-FIX-89` |
| UP-FIX-87 | 🚨 **계기판 5개와 계약 표찰 3개가 네 포즈 전부에서 0픽셀이다.** 화물칸 안쪽을 향하고 있지 않고(단면 재질이라 **벽 뒤에서만 보인다**) `PanelBack`/`ContractPanel` 에 가려 있다. **폰트를 고쳐도 플레이어는 못 본다** | UP-VIS-01, UP-DEVICE-06 | ✅ **조치 완료 · 독립 평가로 확인됨 (2026-08-04, `40461dd`+`d82155a`+`14bc9e9`)** — ⚠ **왼쪽 진단의 후반부(「`PanelBack`/`ContractPanel` 에 가려 있다」)는 완전히 틀렸다.** 신규 차분 렌더 하네스 `Assets/Editor/ReadoutVisibilityProbe.cs`(486줄, 같은 포즈를 대상 렌더러 켜고/끄고 두 번 찍어 화소차를 센다)가 반증했다.<br><br>**진짜 원인은 7개였다**: ① `HideRenderers("…/InstrumentPanel")` 가 TMP 아닌 자식을 **전부** 껐다 — 주석은 「배경 쿼드만」이라 적혀 있었다. 전력 막대 + 임계 눈금 5개 소실 ② 계기판 덩어리가 판독면에서 **(0.26, 0.55, 0.48) 떨어진 허공**에 있었다(자식들이 구 그레이박스 월드 좌표를 로컬로 보유) ③ 계약 명판이 **한 변 1m 정육면체**(`localScale = Vector3.one` + 단위 Cube) — 세 판이 서로를 삼키고 벽을 0.55m 뚫었다 ④ 글자가 `align=Left` + 렉트 폭 17 로 판에서 0.561m 옆 ⑤ **표찰이 180° 반대** — 이 씬 월드 TMP 는 `_CullMode=2` 라 `forward` 반대쪽에서 읽히고 씬 전체가 그 규약인데 계약 표찰만 rotY 270 ⑥ 과적등이 자기 하우징 안 ⑦ **A~D 네 포즈 전부에서 계약 명판이 카메라 뒤** — 씬 결함만이 아니라 **캡처 세트의 구멍**(포즈 `E_contract_wall` 추가로 해소).<br><br>`d82155a` 가 같은 뿌리를 하나 더 찾았다 — `MovePowerReadout` 의 `panel.rotation = Euler(0,180,0)`. 차분 렌더: `rotY 180` → Floor/Power/Required/Status/Cascade **전부 0px**, `rotY 0` → **174/194/252/327/269px**.<br>`14bc9e9` 가 글자 배율을 **0.3393 → 1.0000**(2.95배)로. 폭을 잡아먹던 것은 **화면에 없는 것들**이었다 — 렌더러가 꺼진 `PanelBack`(폭 1.70)이 경계를 지배, `CascadeLabel` 의 `Renderer.bounds` 가 미사용 정점 때문에 0.62 허깨비. TMP `Renderer.bounds` 대신 글리프 상자(`characterInfo[i].isVisible`)로 계측하도록 고침.<br><br>**독립 평가 4차: `계기판본문` = 4점**, 「6줄 전부 판독」. 1차 1점 → 2차 2 → 3차 3 → 4차 **4**.<br><br>⚠ **새로 생긴 결함**: B·F 포즈에서 계기판이 **우측 프레임 밖으로 절단**(`층`, `0 %`, `무게 0/0` 끝자리). 평가자 註 — 「X 에서는 패널이 비어 있어 잘릴 내용이 없었다. 기능적으로는 개선이지만 이 고정 세트에서 새로 생긴 결함이라 회귀로 적는다」 → `UP-FIX-88`<br>~~열림 — 배치·회전 문제라 구조 변경~~ |
| UP-FIX-88 | **계기판이 B·F 포즈에서 우측 프레임 밖으로 절단된다.** `층`, `0 %`, `무게 0/0` 의 끝자리가 잘린다. F 는 `정상` 첫 글자가 레버 팔에 가린다 | UP-VIS-06, UP-DEVICE-06 | ✅ **해소 — 8차 독립 평가가 닫았다 (2026-08-05). 3차부터 열려 있던 항목.** `G_gauge_face` 일곱 줄이 전부 판(x 675~1440) 안에 왼쪽 단으로 정렬됐고, 매니페스트 「계기판 우측 절단」이 **일곱 포즈 전부 `온전`**.<br>고친 것은 셋이다 — ① 글리프 왼쪽 끝을 **재서 보정하던 것을 절대값 표(`PanelRows`)로** 바꿨다(보정량이 그때 그 문구에 딸린 값이라 런타임이 문구를 바꾸면 어긋났다. 고칠 것은 보정량이 아니라 **보정한다는 발상**이었다) ② TMP `margin` 을 0 으로 못 박았다 — 「인스펙터에서 같아 보이는데 화면이 다르다」의 정체 ③ `FloorLabel` 을 두 줄로 접었다.<br>⚠ **세 라운드를 속인 것은 에디트 모드 문구였다** — 고정 캡처의 씬 기본 문구(「0 / 10 층」)가 런타임(「1층 / 10   위험도 안정」)보다 짧아 절단이 18px 로 과소평가됐다. 실제로는 한 줄 **1.095 m** 에 판독면 0.72 m, 즉 판의 1.5배였다.<br><br>~~아래는 해소 이전의 기록이다.~~ ⏳ **부분 조치 · 평가 대기 (2026-08-04)** — 원인을 좌표로 확정했다. **글자 크기 문제가 아니었다.** 판독면 `Screen` 이 월드 X 0.983…1.773 인데 포즈 B 의 프레임 우단은 X **1.447**, F 는 **1.661** 이다. 즉 **계기판은 자기 판독면 안에 있고 판독면이 포즈 안에 없다** — 평가자가 적은 「판독면 폭 또는 포즈 프레이밍」이 맞았다.<br>두 가지를 고쳤다(글자 배율 불변, `UP-FIX-86` 보존): ① 내용을 판독면 **왼쪽에 붙인다**(`MovePowerReadout` 이 `Screen` 경계를 재서 +12mm 여백. 글리프 좌단 1.028 → **0.995**) ② `요구 {N}  {P:P0}` → `요구 {N} {P}%` — 지운 것은 글자가 아니라 **공백 셋**(`{0:P0}` 이 ko-KR 에서 `%` 앞에 공백을 하나 더 넣는다). 가장 넓은 줄 0.6445 → **0.5452 m**.<br>**결과**: F **전부 온전**(이전 `요구`·`무게` 절단) · B `StatusLabel` 온전 · B `RequiredLabel` 123 → **52 px** · B `FloorLabel` 36 → **18 px** · A/C/D 온전. 글리프 높이 불변(A 13~14 · B 29~31 · D 14~16 px).<br>⚠ **B 는 미해결이고 기하학적으로 못 고친다** — 왼쪽 끝에 붙여도 쓸 수 있는 폭이 0.452 m 인데 가장 넓은 줄이 0.545 m 다(런타임 실값은 더 넓다). 남은 축 셋이 전부 막혀 있다: 글자 축소(`UP-FIX-86` 되돌림) · 카메라 이동(고정 세트 비교 불가) · `PowerMeter` 이동(왼쪽에 `ExecutionLeverBase` 0.578…0.938 이 붙어 있다). 대신 **계기판이 온전히 담기는 포즈 `G_gauge_face` 를 추가**했다(글리프 44~64 px, 여섯 줄 전부 판독. 기존 A~F 는 한 값도 안 건드렸다).<br>⚠ **F 의 `정상` 가림은 그대로다** — 카메라와 판 사이에 실행 레버 구조물이 있다. 레버 이동은 형상·배치 결정이라 범위 밖. `G_gauge_face` 에서는 가림 없이 읽힌다.<br>증거 `Captures/symbols_v3_20260804/manifest.txt` 의 「계기판 우측 절단」 표. **`VERIFIED` 로 올리지 않았다**<br><br>**▶ 5차 독립 평가 판정 (2026-08-04): F 해소 / B 미해소 — 열어 둔다.**<br>F 는 `%` 가 프레임 안으로 들어왔다. B 는 `요구 0` → `요구 0 0`+잘린 글리프로 **한 글자 반 늘었을 뿐 여전히 52px 넘친다.**<br>⚠ **평가자가 `G_gauge_face` 를 해결로 인정하지 않았다** — 「계기판이 온전한 화각이지만, 이는 `B-5` 금지 #15 「핵심 결과가 특정 위치에서만 보이는가」를 **해소하는 것이 아니라 증명하는 것**이다」.<br><br><br>**▶ 2026-08-04 (v9) 부분 개선** — `UP-FIX-93` 수정이 이 줄을 함께 줄였다. `요구 {N} {P}%` → `요구 {N}` 으로 토큰을 하나 지우자 **B 절단이 52 px → 18 px**. 다만 넘치는 줄이 `RequiredLabel` 에서 `FloorLabel` 로 **옮겨갔다** — 여전히 미해소다.<br>남은 축은 **판독면을 좁히거나 줄바꿈**뿐이다 (글자 축소는 `UP-FIX-86` 되돌림, 카메라 이동은 고정 세트 파괴, `PowerMeter` 이동은 `ExecutionLeverBase` 가 막는다) |
| UP-FIX-92 | 🚨 **등을 돌리면 전력을 알 수 없다 — 네 라운드 연속 지적, 두 라운드 연속 2점.** `C_toward_gate`·`E_contract_wall` 화각에 전력·요구·층이 **존재하지 않는다.** `gray/C` 판독 글리프 **0개** | UP-SPACE-09, UP-VIS-09 | ✅ **해소 — 7차 독립 평가가 닫았다 (2026-08-05).** C(478~575, y 552~568)·E(570~805, y 458~492) 에 반복기가 들어왔고 **8차에서도 유지**된다. 조명 채널이 아니라 **물리 표시기**를 C·E 화각에 넣은 것이 답이었다 — 5차의 「다음 시도는 세기 조정이 아니라 배치 변경이어야 한다」가 맞았다.<br>⚠ `B-5 #15` 는 **절반만 닫혔다** — C·E 에 `남은 스핀`·`층 상승` 이 아직 0개다(`UP-FIX-104`).<br><br>~~아래는 해소 이전의 기록이다.~~ 🔴 ~~최우선 · 반복 상한 초과~~ (2026-08-04 5차 독립 평가). 2차 「등을 돌린 C 에는 전력 정보가 없다」 → 3차 R7 「E 에서 전력 표시 전무」 → 4차 B-5 → 5차 2점. **`visual-verify` §6 의 「같은 층위 3회 실패면 네 번째를 시도하지 않는다」를 이미 넘었다.**<br><br>**5차가 조명 채널 안을 실측으로 기각했다.** `P-20260804-05` 권장안 B(천장등 색·밝기가 달성률을 따라감)를 구현했으나 셋 다 실패 — ① **값을 못 싣는다** (p060·p100 육안 구분 불가, p000↔p240 mean 차 5%) ② **회색조에서 사라진다** (heavy 프리셋 양 끝조차 구별점 없음) ③ **정반대 상태가 같은 화면** (`p000` 0%·Stable ≡ `p240_collapse` 240%·Collapse, mean .0421 vs .0419 · g/r 1.0411 vs 1.0235).<br><br>**다음 시도는 세기 조정이 아니라 배치 변경이어야 한다** — C·E 화각 안에 **전력 수치를 그리는 물리적 표시기**(글자 또는 게이지). 「조명 색으로는 값을 실을 수 없다」가 실측으로 증명됐다 |
| UP-FIX-93 | **`요구 0 0%` 자간 압착** — A·D·G 에서 `요구`/`0`/`0%` 세 토큰의 간격이 균등해 **요구값과 달성률을 가를 수 없다.** 직전엔 `요구 0   0 %` 로 분리돼 있었다 | UP-DEVICE-06, UP-VIS-09 | ✅ **조치 완료 · 평가 대기 (2026-08-04)** — 공백을 되돌리지 않았다. 되돌리면 폭이 돌아와 `UP-FIX-88`(B 절단)이 커진다 — 두 요구가 같은 줄에서 서로를 배신한다. 대신 **토큰을 하나로 줄였다**: `요구 {N} {P}%` → `요구 {N}`. 혼동이 원리적으로 사라지고 줄이 좁아져 **B 절단이 52 px → 18 px** 로 함께 줄었다.<br>달성률은 신설 `AscentColumn` 이 가져갔다 — 물리 탱크의 **채움 높이**(회색조 실측 0/17/36/57/85%)와 「전력 N   P%」 한 줄. 회색조에서 살아남는 쪽으로 옮긴 것이다.<br>⚠ 대가: `G_gauge_face`·`F_board_front` 에서 달성률 숫자가 계기판에 없다. 탑이 그 화각에 부분적으로만 들어온다. **`VERIFIED` 로 올리지 않았다** |
| UP-FIX-94 | **신설 포즈 `G_gauge_face` 가 `VISUAL_SPEC` §12 수치 대역을 셋 위반한다** — p99 **.9033**(대역 .25~.36, 2.5배) · &gt;0.50 **1.59%**(≤0.5%, 3.2배) · &lt;0.02 **34.33%**(≤32%). 이 저장소가 기록한 것 중 가장 밖이다 | UP-VIS-01, UP-VIS-02 | ⏳ **부분 조치 · 평가 대기 (2026-08-04)** — 3 위반 → **2 위반**. p99 **.9033 → .4405** · `>0.50%` **1.59% → 0.36%(대역 안)** · `<0.02%` **34.33% → 40.63%(악화)**. 원인은 `UP-FIX-90`(방 안 최고 백색이 계기 텍스트)과 **같은 것**이고 이 포즈에서 최대치가 됐다. `gray/G` 에서 글자가 **순백으로 타고 헤일로가 붙는다.**<br>⚠ A/C/D 집계는 6/7 로 통과한다 — **총량 지표가 이 결함을 못 잡는다**는 `UP-FIX-90` 의 지적이 포즈 단위에서 재확인됐다. 스타일 「§12 수치 대역」이 4 → **3 으로 하락** |
| UP-FIX-89 | **정보 위계가 뒤집혀 있다.** B 기준 과수확 35px &gt; 전력 31px &gt; **실행 24px** 인데 `VISUAL_SPEC` §5 는 실행이 **1순위**다. 또한 A/D 의 초점 덩어리가 3×3 보드에서 **백색 6줄 숫자 블록**으로 이동했고, 블록 내 7개 항목이 전부 같은 크기·같은 백색이라 §5 「한 화면에 모든 숫자를 동시에 강조하지 않는다」에 걸린다 | UP-VIS-01, UP-VIS-09 | ⏳ **부분 조치 · 평가 대기 (2026-08-04)** — 평가자 말대로 **크기가 아니라 밝기·배치**로 풀었다. ① 과수확 표찰(잠김)을 순백 1.000 → **.388** 로 내렸다 — 잠긴 선택지가 사용 가능한 선택지보다 밝을 이유가 없다. 해제되면 `AscentColumnView` 가 같은 프레임에 붉게(0.94,0.42,0.24) 살린다. ② 실행 표찰을 **.837** 로 올려 방에서 가장 밝은 글자로 만들었다. ③ 신설 `AscentColumn` 이 계기판의 여섯 필드 중 1순위 둘(전력량·남은 스핀)을 **큰 형상**으로 빼내고 나머지를 어둡게 해 「전부 같은 크기·같은 백색」을 깼다.<br>**실측 위계**(설정 잉크 휘도): 실행 .837 &gt; 탑 큰숫자 .530 &gt; 계기판 .465 &gt; 과수확 .388.<br>⚠ **면적은 여전히 탑이 크다**(A 681 px vs 실행 304 px). 숨기지 않는다 — 사용자가 「한눈에 보고 결정」을 요구했으므로 계기가 2글자 표찰보다 큰 것은 의도한 것이다. **크기로 풀 수 없다.** 「실행」을 더 키우면 「글자가 레버 실물보다 무겁다」는 직전 지적과 충돌한다. **위계를 맞추려면 레버 형상 쪽을 키우거나, 숫자 블록 내부에 강약을 넣어야 한다** — 값이 아니라 배치·형상 결정 |
| UP-FIX-90 | **방 안 최고 백색이 계기 텍스트가 됐다.** 레퍼런스는 「붉은색만 강조색 / 계기 숫자도 붉다」인데 반대가 됐다 | UP-VIS-01, UP-VIS-02 | ⏳ **조치 완료 · 평가 대기 (2026-08-04)** — 실측으로 원인을 확정했다: 계기 라벨 다섯 줄이 전부 `(1,1,1)` 이었고 **TMP 는 비조명 셰이더**라 그 값이 곧 화면 값이다. §12 mean .044 인 방에서 유일한 순백이었다.<br>**크기가 아니라 밝기를 내렸다**(`UP-FIX-86` 되돌림 금지). 1순위 보조 1.000 → .566 / 2순위 1.000 → .396 / 계약 .834 → .545 / 과수확 1.000 → .388. 잉크는 전부 따뜻하다(r &gt; g &gt; b) — §12 색조 축과 같은 방향이다.<br>**결과**: 방에서 가장 밝은 글자가 **실행 표찰**(설정 잉크 .837 · 렌더 최대 .828)이 됐다. `VISUAL_SPEC` §5 1순위와 일치한다. `G_gauge_face` p99 **.9033 → .4405**, `>0.50%` **1.59 → 0.36(대역 안)**.<br>⚠ 남은 것: A/C/D p99 **.2335**(대역 .25~.36, v3 도 밖) · `G` `<0.02%` **34.33 → 40.63 으로 악화**(글자를 어둡게 한 대가). 근거 `Captures/symbols_v4_20260804/manifest.txt`.<br>⚠ **총량 지표로는 안 잡힌다.** 평가자 註: 「g/r 총량 지표(.8760, 대역 안)는 면적이 작아 이 문제를 잡지 못한다」. 즉 §12 수치 대역을 통과해도 이 결함은 남는다 — **대역 검사에 구멍이 있다는 증거이기도 하다** |
| UP-FIX-91 | **백로그가 구현·평가 실제 상태보다 8커밋 뒤처져 있었다.** `UP-FIX-80·82·83·86·87` 이 실질 해결됐는데 전부 「열림」이었고, `UP-FIX-87` 의 기록된 추정 하나는 차분 렌더로 **반증됐는데도** 원문에 남아 있었다 | (문서 정합성) | ✅ **해소 (2026-08-04)** — 위 다섯 항목을 「조치 완료 · 독립 평가로 확인됨」 또는 「부분 조치」로 갱신하고 근거·커밋·남은 미달을 함께 적었다. **`VERIFIED` 로 올리지 않았다** — 구현자가 자기 작업을 승인하지 않는다(백로그 §0.4).<br>**재발 방지**: 구현 커밋은 백로그 상태 칸을 함께 고친다. 「열림 → 조치 완료·평가 대기」는 자기 승인이 아니므로 구현자가 해도 된다 |

| UP-FIX-95 | 🚨 **같은 프레임의 두 전력 표시가 다른 값을 낸다.** 전력 스윕 10장 전부 — 계기탑 `전력 516  240%` / 벽 계기판 `전력 0` `요구 0`. **요구 전력 215 는 여덟 포즈·열 스윕 어디에도 그려지지 않는다** | UP-DEVICE-06, UP-POWER-02 | 열림 (2026-08-04 6차 독립 평가, **회귀**) — 「지금 통과 가능한가」를 화면으로 답할 수 없다. `MASTER_PRD` §17.2 가독성 완료 조건의 직접 위반.<br>⚠ **런타임 불일치인지 스윕 하네스가 탑만 구동한 결과인지 캡처로 판별 불가.** 어느 쪽이든 제출된 증거가 모순돼 있다 — **먼저 어느 쪽인지 가려라.** 벽 계기판이 구동된 플레이 모드 프레임 1장이면 갈린다 |
| UP-FIX-96 | **계기탑 캡션·예비줄이 판독 하한 미달.** `스핀 남음`·`층 상승` 캡션 8px(A)/9px(D)/10px(H), 예비줄 `배수 0.00배 손실 —` 11/13/15px. 자인 한글 하한 16px | UP-VIS-09 | ✅ **해소 — 7차 독립 평가가 닫았다 (2026-08-05).** 캡션·예비줄이 자인 하한 16px 를 넘겼다.<br><br>~~아래는 해소 이전의 기록이다.~~ 열림 (2026-08-04 6차 독립 평가) — **기본 선 자세 A 에서 두 큰 숫자의 라벨이 뭉개져 `5`·`0` 이 무엇의 값인지 화면만으로 알 수 없다.** 숫자는 35~47px 로 잘 읽히는데 캡션이 죽어 의미가 안 붙는다. H 에서만 겨우 읽힌다 |
| UP-FIX-97 | **계기판 여섯 필드의 시각 무게가 여전히 균일하다.** `0/10 층 · 전력 · 요구 · 무게 · 정상 · 연쇄` 가 한 크기·한 색 | UP-VIS-09, UP-DEVICE-06 | 열림 (2026-08-04 6차 독립 평가) — 사용자가 위계를 확정했는데(1순위 = 전력량·남은 스핀·레버·3×3) **이번에 한 것은 선택적 강등이 아니라 패널 전체의 균일 감광(백색 → 호박색)이다.** 5차 지적의 구조가 그대로 남았다. **필요한 것은 밝기 일괄 하향이 아니라 필드별 크기·색 분리** |
| UP-FIX-98 | **신설 포즈 `H_lever_column` 이 §12 대역 3개 위반** — mean .0451(하한 .055) · p50 .0317(하한 .040) · &lt;0.02 34.56%(≤32%). 그리고 `G_gauge_face` 의 &lt;0.02 가 34.33 → **40.63%** 로 악화, mean 이 하한 밑으로 | UP-VIS-01, UP-VIS-02 | 열림 (2026-08-04 6차 독립 평가, **회귀**) — `UP-FIX-94`(하이라이트 폭주)를 잡은 **대가로 검정 화소가 늘었다.** 5차가 G 에 대해 지적한 유형이 신설 포즈에서 반복됐다 — **새 포즈를 추가할 때 §12 대역을 먼저 확인하지 않는 패턴**이 두 라운드 연속이다 |
| UP-FIX-99 | **위험 4단계 판정 근거가 0장이 됐다.** `Stable ↔ Strain ↔ Critical ↔ Collapse` 를 가를 캡처가 최신 세트에 없다 | UP-RISK-03, UP-VIS-06 | 열림 (2026-08-04 6차 독립 평가) — 조명 사다리 채널을 폐기하면서 **위험 상태 프레임도 같이 사라졌다.** 직전 세트에는 최소한 `p240_collapse` 가 있었다. `VISUAL_SPEC` §6 「Stable 과 Critical 의 감정 차가 명확한가」가 **여섯 라운드째 판정 불가**다.<br>→ 네 위험 상태 각각의 **동일 포즈** 캡처가 고정 세트에 들어가야 한다 |
| UP-FIX-108 | **캡처 측정기가 프러스텀 포함을 확인하지 않는다.** 매니페스트 「운행 계기탑 글리프」 표의 `G_gauge_face` 행이 `스핀숫자 152px … 탱크관 450px` 로 적혀 있으나 **그 이미지에 계기탑이 아예 없다** | UP-VIS-06, UP-TEST-10 | 열림 (2026-08-04 6차 독립 평가) — 같은 표가 `C`·`E` 는 `화각밖` 으로 정확히 판정하므로 **G 행만 투영 크기만 계산하고 화각 포함을 안 봤다.** 평가자가 이미지를 채택해 판정은 지켰지만, **측정값이 이미지와 어긋난 채로 제출됐다는 사실 자체가 증거 신뢰도 문제**다. 고치지 않으면 다음 라운드 수치를 믿을 수 없다.<br><br>⚠ **ID 정정 (2026-08-05)**: 이 결함은 6차가 `UP-FIX-100` 으로 열었는데, 7차 판정문이 **같은 번호를 다른 결함(실행 레버 기구 형상)에 다시 배정**했고 8차가 그 용법을 이어받았다. 한 ID 가 두 결함을 가리키면 「`UP-FIX-100` 해소」라는 문장이 무엇을 해소했는지 특정할 수 없다. 두 라운드에 걸쳐 쓰이고 **최우선으로 지정된 레버 쪽에 `100` 을 남기고**, 한 번만 인용된 이쪽을 `108` 로 옮겼다 |
| UP-FIX-100 | **`실행` 레버에 기구 형상이 없다 — 라벨만 있고 몸이 없다.** 그리고 `과수확` 과 같은 세로 기둥에 라벨 둘이 붙어 있다 | UP-VIS-09, UP-DEVICE-06 | 🔴 **최우선 · 반복 상한 초과 (2026-08-05 8차 독립 평가 · 6라운드 연속)** — 8차에서 그립이 `실행` 밑으로 오긴 했다(gray/H x 940~985, y 688~752 · gray/A x 1200~1240, y 545~585). **그러나 `과수확` 이 그 대가로 몸을 잃었고**, 이제 라벨 둘이 한 기둥에 65~135px 간격으로 앉아 있다(A x 1195~1265 · H x 905~1010). `F_board_front` 에는 `실행` 아래 기구가 아예 없다. 7차의 「레버 형상은 하나인데 라벨은 둘이다」가 **주어만 바뀐 채 그대로 성립한다.**<br><br>⚠ **여기서 두 동결 문서가 충돌한다 — 값 조정으로 못 푼다.** 평가자가 요구한 「`실행` 그립과 **가로로 분리된 x 좌표**」의 근거는 `VISUAL_SPEC` §7(덮개·잠금 장치 **또는 별도 위치**)인데, `MASTER_PRD` §7 은 2026-08-02 에 **「물리 레버는 하나다」**로 확정됐다. 문서 우선순위상 `MASTER_PRD` 가 이긴다.<br>이 충돌의 뿌리는 `DECISION_LOG` 였다 — `D-20260730-08`(과수확 레버 신설)이 `D-20260802-10` 에 뒤집힌 뒤에도 **`Accepted` 로 남아 사흘 동안 정반대 지시가 둘 다 유효해 보였다** (2026-08-05 `Superseded` 로 정정).<br>**진행 가능한 해석**: 레버 **하나**에 2단을 형상으로 드러낸다 — 아래 단 그립 + 위 단 덮개/걸쇠, 잠금 해제 전엔 물리적으로 막혀 보이는 형태. 그러면 §7(하나)과 `VISUAL_SPEC` §7(덮개·잠김)을 동시에 만족하고 「같은 세로 기둥 금지」 한 줄만 못 지킨다. **그 한 줄을 평가 기준에서 조정할지는 사용자 결정** → `P-20260804-01` 과 함께 푼다.<br>**라벨 좌표를 다시 옮기는 시도는 하지 않는다** — 일곱 번째다 (`visual-verify` §6) |
| UP-FIX-101 | **탑 전력줄 「전」 글자 겹침** | UP-VIS-09 | ✅ **해소 — 8차 독립 평가가 닫았다 (2026-08-05).** A·B·D·H 네 포즈 전부 「전력 0  0%」 온전 |
| UP-FIX-102 | **`D` 좌단 반복기 프레임 클리핑** · 측정기에 클리핑 확인 추가 | UP-VIS-06, UP-TEST-10 | 열림 (2026-08-05 8차) — **도구는 신고하지만 화면은 그대로다.** `D` 좌단은 여전히 「0  0%」 |
| UP-FIX-103 | **탱크 임계 리브에 값 각인이 없다** (`B-3 #9` 나머지 절반) | UP-POWER-02, UP-VIS-09 | 열림 · **같은 층위 4라운드 연속 (5·6·7·8차)** — 채움이 붉은 리브를 넘는 것은 보이는데 **리브에 값이 없어** 100 인지 170 인지 화면이 말하지 않는다. **리브 형상은 이미 있다. 남은 것은 각인 텍스처 또는 지오메트리 추가** — 값 조정이 아니다 |
| UP-FIX-104 | **`C`·`E` 에 `남은 스핀`·`층 상승` 이 0개** (`B-5 #15` 잔여) | UP-SPACE-09, UP-VIS-09 | 열림 · 2라운드 연속 (7·8차) — `UP-FIX-92` 가 전력·요구·층을 C·E 에 넣어 닫혔지만 **나머지 두 필드는 아직 그 화각에 없다** |
| UP-FIX-105 | 🔴 **`실행`/`과수확` 두 라벨이 같은 기둥에서 분리되지 않았다** | UP-VIS-09, UP-DEVICE-06 | 열림 (2026-08-05 8차 신규 · **최우선**) — `UP-FIX-100` 과 **같은 작업으로 풀린다.** 라벨 이동이 아니라 `VISUAL_SPEC` §7 이 여섯 줄로 적어 둔 것을 **형상으로** 만드는 일이다(더 크고 무겁다 · 덮개나 잠금 장치나 별도 위치 · 달성 전에는 물리적으로 잠겨 보인다 · 붉은색 하나로만 표현하지 않는다). **여섯 라운드 동안 이 여섯 줄 중 화면에 구현된 것은 0개다** |
| UP-FIX-106 | 🚨 **월드 라벨이 계기판 본문을 덮는다.** `F_board_front` 에서 `실행`(x 1613~1690, y 232~268)이 계기판 첫 줄 「1층 / 10」(x 1588~1755, y 258~290)을 덮어 **「1[실행]10」** 으로 읽힌다. `B_machine_front` 에서는 「전력 0 / 요구 0」 두 줄을 덮는다 | UP-DEVICE-06, UP-TEST-10 | 열림 (2026-08-05 8차 신규 · **회귀**) — `F` 는 `VISUAL_SPEC` §9 가 지정한 정면 캡처 포즈다.<br>⚠ **구조적 원인: 매니페스트에 겹침을 재는 항목이 하나도 없다.** 클리핑·프러스텀·글리프 높이·기여 화소는 재는데 **글리프가 글리프를 가리는 것**은 안 잰다. 그래서 이 결함은 「일곱 포즈 전부 온전」이라는 보고와 **동시에 참**이다. 직전 라운드에 고친 「측정기가 잘린 것을 온전하다고 셌다」(`UP-FIX-88`)와 정확히 같은 종류의 사각지대다.<br>→ **측정기에 글리프 상자 대 글리프 상자 겹침 검사를 넣지 않으면 다음 라운드도 같은 방식으로 통과 보고가 나온다** |
| UP-FIX-107 | **방에서 가장 밝은 큰 덩어리가 우벽 무텍스처 상자와 바닥이다.** `gray/A` 에서 3×3 보드 9개 원반이 우벽 선반·상자(x 1490~1920, y 620~1080)와 바닥 다이아플레이트보다 **어둡다**. `gray/E` 에서는 좌하단 궤짝(350~790, y 765~1015)이 주제인 계약 3판을 이긴다 | UP-VIS-01, UP-VIS-02 | 열림 (2026-08-05 8차 신규) — 레퍼런스가 요구한 「선반 + 상자」는 **배치로는 맞는데 값이 반대**다. `G`·`H` 의 §12 mean·p50 이 세 라운드째 하한 밑인 것(`UP-FIX-98`)과 **같은 원인의 뒷면**이다 — 빛이 정보에 안 가고 소품에 가 있다.<br>**고칠 층위: 조명 배치와 알베도 값 배분.** 포스트 처리 값 조정으로는 세 라운드 실패했다 |
| UP-FIX-109 | 🚨 **`D-20260805-01` 이 채택한 Mercy / Hunger 규칙 계층 전체가 실행 경로 0곳이다** | (추적 부재) | 열림 (2026-08-05 · 요구사항 감사 F-3 · 아키텍처 감사 R-1 이 **독립적으로 같은 결론**) — 구현물은 있다(`Run/MercyHunger.cs` 150줄 · `Data/Profiles/MercyHungerProfile.cs` 77줄 · 테스트 13건). 그런데 비테스트 호출자가 **`MercyFor` 0곳 · `HungerFor` 0곳**, `FloorSession.SpinsRemainingAtFirstReach`(`:387`)·`ExcessRatio`(`:390`) **0곳**, `MercyHungerProfile.asset` **파일 자체가 없다**. `FloorResult`·`FloorRecord` 에 등급이 없다.<br>`FloorSession.cs:383` 주석은 이 필드를 「Mercy 등급의 **유일한 입력**」이라 적는데, 그 입력을 소비하는 코드가 없으므로 등급은 런타임에 **한 번도 계산되지 않는다.**<br><br>⚠ **이 저장소가 이름 붙여 기록해 둔 실패의 재발이다** — `RemainingSpinSettlementProfile`(`FloorSession.cs:193-200`)의 주석이 「프로파일 8종이 여덟 세션 동안 죽어 있는 것을 못 봤다」고 적어 둔 **바로 그 자리에 9번째가 들어왔다.** `PowerBand.Damaged`·`PendingSettlementMoney` 와 같은 모양이다.<br>**586 PASS 는 「순수 함수가 옳다」만 증명하고 「게임이 그것을 쓴다」는 증명하지 않는다.**<br><br>커밋 `c94773d` 메시지가 미완을 정직하게 적었다는 점은 인정한다(「아직 없는 것: 층 종료 선택 UI · Hunger 보상 4종 · FloorResult 에 등급 노출」). **문제는 그 미완이 어디에도 추적되지 않았다는 것**이다 — 이 줄이 생기기 전까지 백로그·`PENDING_DECISIONS` 양쪽에 `Mercy`·`Hunger` 가 **0건**이었다.<br><br>⚠ **이 줄은 추적일 뿐 Required 가 아니다.** Required 집합을 늘리는 것은 **완료 목표의 크기를 바꾸는 범위 결정**이라 사용자 몫이다(`PD-19` 와 같은 성질). 최소 배선은 싸다 — `FloorResult` 에 필드 둘 + `FloorRecord.Summary()` 한 줄이고 `FloorResult` 생성자 호출부는 `FloorSession.Resolve`(`:702`) 하나다. 다만 `FloorSession`/`RunSession` 의 **8단 생성자 위임 체인**에 임계치 프로파일까지 얹히면 비용이 몇 배가 되므로 그 정리가 먼저다 |
