...
# 미구현 확인 — `PowerBand.Jettison` (2026-07-31, 5차 감사에서 드러남)

독립 감사가 `17_accident_recorder`를 "자기모순"으로 판정했다.

  층 실패 · 화물 포기
  적재 없음   무게 0/100
  위험 최고 안정 (위험 요인 없음)

"화물을 포기했다는데 적재가 없다"는 지적이었다. 렌더링 문제로 보고 기록 시점을
의심했지만, 코드를 따라가니 원인이 달랐다.

`PowerBand.Jettison`(요구의 70~89%)은 열거형 주석에 **"승객·화물 포기 또는 큰 대가"**
라고 선언돼 있다. 그런데 `RequiresJettison`의 유일한 사용처가 이것이다.

  AscendResult.cs:66 — (RequiresJettison ? "Jettison required" : string.Empty)

**실패 문자열을 만드는 데만 쓰인다.** 적재를 비우는 코드도, 승객을 내리는 코드도,
대가를 물리는 코드도 없다. 게임이 "화물 포기"라고 말한 뒤 아무 일도 일어나지 않는다.
적재가 없는 런에서는 문구 자체가 뜻을 잃는다.

기록기는 정확했다 — 적재가 실제로 없었고(그 런은 `AddWeight`만 썼다), 최고 위험도도
실제로 Stable 이었다(전력 부족으로 실패했지 과적·과수확으로 실패한 것이 아니다).
**모순은 화면이 아니라 규칙에 있다.**
...
이 세션에서 고치지 않는다. 이유:
  ...
  · 무엇을 얼마나 버릴지는 밸런스 결정이다 — `MASTER_PRD` §5 도 수치를 주지 않는다.
  · 적재가 없을 때의 대체 대가(소지금? 다음 층 요구 증가?)도 정해진 바 없다.
  · `CLAUDE.md` — "승인 대기 비주얼·모션·밸런스는 하나의 최종안으로 잠그지 않는다".

→ `DECISION_LOG`에 결정 요청으로 올린다. 그때까지 화면 문구는 실제 동작과
   어긋나므로, 최소한 **일어나지 않는 일을 일어났다고 적지 않도록** 문구를
   고치는 것이 다음 세션의 첫 작업이다.

---

# 위 「미구현 확인 — `PowerBand.Jettison`」 절은 무효다 (2026-08-01 확인)

그 절은 "결정 전까지는 구현하지 않는다"로 끝난다. **그 뒤 구현됐다.**
이 세션에서 코드로 재확인했다.

| 그 절의 주장 | 현재 사실 |
|---|---|
| `RequiresJettison`의 유일한 사용처가 실패 문자열 생성이다 (`AscendResult.cs:66`) | 그 행은 더 이상 없다. 현 `AscendResult.cs:67`은 `FailureReason = RunEnded ? "Crash" : string.Empty` 이며, `Jettison`은 실패가 아니다 |
| 적재를 비우는 코드도, 대가를 물리는 코드도 없다 | `RunSession.cs:265`가 `PayJettisonCost()`를 부르고 `:306-339`가 실제로 물린다 |
| 게임이 "화물 포기"라고 말한 뒤 아무 일도 일어나지 않는다 | 적재가 있으면 가장 무거운 것부터 절반 이하까지 버리고, 없으면 소지금 40%를 잃는다 |
| 테스트가 없다 | `BuildTests.cs:55` `화물 포기 구간은 런을 끝내지 않고 대가를 물린다` |

`docs/runtime/VisualGapAnalysis.md` 5차 감사 §A("규칙의 문제 — 화면이 아니다")도
같은 이유로 무효다. `17_accident_recorder`의 "화물 포기 / 적재 없음"은 이제
"실은 것이 없어 소지금으로 대신 냈다"는 정상 동작을 뜻한다.

`DECISION_LOG` `D-20260731-04 상태 갱신`에 세 갈래 규칙 전문이 있다.
남은 미결은 하나뿐이다 — **승객과 화물을 구분해서 버릴 것인가.**

# 커리큘럼 매핑 갱신 (2026-08-01)

위 「Phase 2 요구사항 매핑」의 층 배치 서술은 08/99번 기준이다.
정본이 03번으로 바뀌었다 — `DECISION_LOG` `D-20260801-01`.
층별 실제 방문률은 `Logs/curriculum_coverage.txt`가 시드 200개로 기록한다.
