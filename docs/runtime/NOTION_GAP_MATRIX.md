...
| 18 | 감각적 위험 상태 시스템 | `UP-RISK-01`~`09` | **부분** | 상태기계·동기화·과수확 VERIFIED / **조명·진동 CONNECTED · 음향·Collapse 연출·프로파일 SKELETON · 접근성 NOT_STARTED** |
| 19 | 승객 상황 반응 | `UP-NPC-01`~`05` | **부분** | 위험 반응만 CONNECTED / **이벤트 10종 중 1종 · ReactionSet 없음 · 동시 반응 제한 없음** |
| 20 | 사고 기록기 | `UP-REC-01`~`05` | **부분** | 기록 생성 VERIFIED / **물리 장치 형태 아님(오버레이 텍스트)** |
| 21 | 디버그 패널 · 결정론적 시드 · 텔레메트리 | `UP-TEST-06`, `UP-CORE-01`·`UP-RUN-05`, `UP-TEST-05` | **부분** | 시드 VERIFIED · 패널 CONNECTED / **텔레메트리 NOT_STARTED** |

**21항목 중 완전 충족 15 · 부분 충족 6 · 미착수 0.**

가장 큰 미충족은 #21의 **텔레메트리**다. PRD §4.1에 명시적으로 적혀 있고 §16.2가
20개 필드를 지정하는데 **코드에 존재하지 않는다.**
