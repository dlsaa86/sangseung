...
> ## ⚠ 두 번째로 먼저 읽어야 하는 문장 — 게이지 홈은 **§10 에서 해결됐다**
>
> §9.4 는 「UV 를 안 고쳐서 재굽기가 무의미했다」고 결론냈다. **절반만 맞았다.**
> UV 는 실제로 낡아 있었지만 그 크기가 하우징에서 **1.7mm·6.2mm** 라 눈에 보이는
> 증상을 만들 수 없다. 진짜 범인은 **`BakedAO/AO_SM_Gauge_Housing.png`** 였다 —
> 알베도가 아니라 **AO** 이고, `AD53_BAKEALL` 이 아니라 **`ad58_bake_ao.py`** 가 굽는
> 다른 파이프라인이라 §9 의 재굽기가 애초에 건드리지 않는 파일이었다.
> **§10 을 읽어라.** 이 문서를 보고 UV 만 다시 파면 같은 자리를 세 번째로 파게 된다.
...
**부수 발견**: 이전 `BK_SM_Gauge_Housing.png`는 1024×1024였는데 당시 돌린
`AD53_BAKEALL`의 `SIZES` 딕셔너리는 512로 지정돼 있었다(`build_cabin.py`의 export
인자가 실제 export 절차와 어긋났던 것과 같은 계열의 함정). team-lead 지시로
`SIZES`를 무시하고 Housing 1024·Labels 1024·Fill 256으로 재굽기했다(§9.4).

FBX는 다시 export하지 않았다 — 지오메트리가 바뀌지 않았으므로(지시대로).

### 9.4 ⚠ 1024 재굽기 후 발견 — Housing 새 파일이 옛 파일과 MD5까지 완전히 같다

**이게 이 섹션에서 가장 중요한 발견이다.** 해상도를 1024로 다시 구운 뒤
일상적인 무결성 확인(MD5)을 했는데:

```
BK_SM_Gauge_Housing.png   새 파일 MD5 == Aug 8 원본(그루브 넓히기 전) MD5   완전 동일
BK_SM_Gauge_Fill.png      새 파일 MD5 == 재굽기 전 백업 MD5                완전 동일
BK_SM_Gauge_Labels.png    새 파일 MD5 ≠ 백업 MD5                          다름(원인 미상)
```

`Housing`과 `Fill`은 **정점을 옮긴 대상인데도** 재구운 텍스처가 옛날 것과
바이트 하나 안 다르다. DIFFUSE 굽기는 간접광을 꺼서 노이즈 없는 결정론적
계산이므로 우연히 같아질 수 없다.

**원인 가설**: 그루브 정점 이동(그루브 확장·Fill 100% 연장) 두 세션 모두
**`.co`(3D 위치)만 바꿨고 `UVBox`/`UVUnique` UV 좌표는 한 번도 갱신하지
않았다.** `NG_Elev_Aged60`의 녹·때·모서리 마모는 이 UV로 텍스처를 샘플링하므로,
셰이더 입장에서는 정점이 3D 공간에서 어디로 움직였는지와 무관하게 **UV 좌표가
그대로면 출력(구운 색)도 그대로**다. 즉:

> **재굽기(bake)는 문제의 절반만 고친다.** 굽기 자체는 매번 새로 정확하게
> 실행됐지만, 굽는 **대상 데이터(UVBox/UVUnique)가 애초에 정점 이동 때 함께
> 갱신된 적이 없어** 몇 번을 다시 구워도 같은 결과가 나온다. 진짜 고치려면
> 그루브의 16개 정점(과 Fill의 이동된 4개 정점)에 대해 UVBox/UVUnique 좌표도
> 새 위치에 맞게 재계산해야 할 가능성이 크다 — **이건 정점 이동보다 델리케이트한
> 작업이라 이 세션에서는 시도하지 않고 team-lead 판단을 기다렸다.**

`Labels`가 왜 달라졌는지는(정점을 안 건드렸는데도 MD5가 바뀜) 조사하지 못했다 —
이 비대칭이 위 가설을 완전히 뒷받침하지는 않는다는 점도 함께 남긴다.

> **2026-08-09 해소.** `Labels` 비대칭은 가설의 약점이 아니었다. 되돌리기 전후 두
> 파일을 픽셀 단위로 대조하니 **1,048,576 화소 중 3 화소, 최대 편차 4** 다 —
> 내용 변화가 아니라 굽기·PNG 인코딩의 반올림 잡음이다. 재현도 된다(§10.2).
> **MD5 는 「1비트라도 다른가」만 답한다. 그것을 「내용이 달라졌다」로 읽으면
> 3화소가 가설 하나를 흔든다.** 대조는 해시가 아니라 화소로 한다.

---

## 10. 진짜 원인 — 알베도가 아니라 **AO** 였다 (2026-08-09 해결)

§9 는 옳은 방향을 봤지만 **틀린 파이프라인**을 봤다. `AD53_BAKEALL`(알베도)만
계속 다시 구웠는데, 게이지 홈의 검정을 만드는 것은 **`ad58_bake_ao.py`(AO)** 였다.
두 파이프라인은 출력 폴더부터 다르다 — `BakedAD47/` vs **`BakedAO/`**. §9 가
몇 번을 재굽기해도 문제의 파일을 **한 번도 건드리지 않았다.**

### 10.1 픽셀 증거 — 홈 바닥면(poly 208) 세로 프로파일

`UVBake` 아일랜드(U 0.720~0.746 · V 0.475~0.872)를 가로로 평균 내 0%→100% 를 쟀다.

| 구간 | 알베도 `BK_SM_Gauge_Housing` | **AO `AO_SM_Gauge_Housing`** |
|---|---|---|
| 0~60% | ~10.0 (평탄) | **0** (완전 가려짐) |
| 65~95% | ~10.0 (평탄) | **~220** (열림) |
| 전체 std | 0.13 | **105.5** |

**알베도는 끝까지 균일하다 — §9.3 의 측정이 옳았다.** 어긋난 것은 AO 뿐이고,
전환점이 62% 다. 사용자가 본 「홈이 100 눈금까지 안 감」이 이 한 계단이다.

### 10.2 왜 AO 만 낡았나 — 굽기 입력이 서로 다르다

|  | 굽는 것 | 입력 | 정점을 옮기면 |
|---|---|---|---|
| 알베도 (`AD53`) | `NG_Elev_Aged60` 의 출력색 | **UV**(`UVBox`/`UVUnique`) | UV 가 그대로면 **출력도 그대로** → MD5 동일 |
| AO (`AD58`) | 반구 레이캐스트 | **지오메트리** | **반드시 바뀐다** |

그래서 §9.4 의 「MD5 가 같다」는 알베도에서는 **정상 동작**이었다. 이상한 것은
AO 를 아무도 다시 굽지 않았다는 사실이었고, 타임스탬프가 그것을 말하고 있었다:

```
AO_SM_Gauge_*.png        Aug 8 19:53     ← 두 지오메트리 수정보다 앞선다
그루브 정점 수정          Aug 9 02:17
SM_Gauge_Fill 100% 연장   Aug 9 03:47
```

### 10.3 무엇이 홈을 검게 만드는가 — 홈이 아니라 **Fill 막대**다

AO 를 세 조건으로 구워 갈랐다. 이 대조가 결론을 확정한다.

| 조건 | 0~60% | 65~90% | 읽히는 모습 |
|---|---|---|---|
| 현재(Aug 8) | 0 | ~220 | 62% 에서 끊긴다 — **버그** |
| 재굽기 · **Fill 보임** | ~10 | ~10 | **끝까지 균일하게 어둡다** ✅ |
| 재굽기 · Fill 숨김 | ~114→207 | ~216 | 홈이 **밝아진다** ❌ |

`SM_Gauge_Fill` 은 홈 바닥면(world Y 2.88118) 앞 **8mm**(Y 2.8733~2.9179)에 떠 있다.
바닥의 검정은 홈이 깊어서가 아니라 **막대가 코앞을 막아서** 생긴다 — 막대를 빼면
얕은 홈의 자기 차폐뿐이라 AO 가 216 까지 밝아진다. 그래서 **Fill 을 보이게 둔 채**
굽는 것이 맞고, Fill 이 이제 0~100% 를 덮으므로 차폐도 끝까지 균일해진다.

⚠ **이건 되풀이될 수 있는 함정이다.** 런타임에 `localScale.x` 로 길이가 변하는
오브젝트의 차폐를 정적 텍스처에 굽고 있다. **`SM_Gauge_Fill` 의 저작 길이를 다시
바꾸면 이 AO 는 다시 낡는다.** 그때는 §10.3 표를 다시 만들지 말고 그냥 AO 를 다시 구워라.

### 10.4 UV 는 실제로 낡아 있었다 — 다만 크기가 다르다

§9.4 의 가설 자체는 맞았다. `UVBox = (P[a], P[b])`, `P = world + (0, −0.70308, 0)`,
축쌍은 면 법선의 우세축(`AD21_FRAME`·`AD35_RAILS` 의 박스 투영 규칙)으로 재현된다.
그 식으로 전수 검사한 결과:

| 메시 | 낡은 loop | 어긋난 양 | 알베도 재굽기 차이 |
|---|---|---|---|
| `SM_Gauge_Housing` | **24** (poly 208·241·242·244·245) | 1.7mm · 6.2mm | 0.82% 화소, 최대 11 |
| `SM_Gauge_Fill` | **6** (poly 0·2·4) | **396mm** | **17.76% 화소, 최대 27** |
| `SM_Gauge_Labels` | 0 | — | 3 화소(잡음, §9.4 주석) |

**하우징의 6mm 로는 증상이 안 나온다.** 그 위의 알베도가 std 0.13 으로 평탄해서
어긋날 무늬 자체가 없다. 반면 **Fill 은 396mm** — 낡은 UV 가 옛 길이(61.7%)를
그대로 담고 있어 무늬가 1.62배로 늘어나 있었다.

⚠ **`UVUnique` 를 박스 투영으로 다시 만들지 마라.** 8,238 loop 전부에서
`UVUnique ≠ UVBox` 다 — `AD21_FRAME`/`AD35_RAILS` 는 둘을 같은 값으로 **생성**하지만
그 뒤 별도로 언랩됐다. 덮어쓰면 언랩이 파괴된다. Fill 의 `UVUnique` 는 U 축만
**같은 아일랜드의 다른 축이 쓰는 uv/m(0.08863)** 로 다시 재어 늘렸다 —
고치기 전 U 0.0547/m vs V 0.0887/m 로 비가 정확히 **0.617**(옛 길이 비율)이었다.
하우징의 `UVUnique` 는 U 0.08795 / V 0.08874(0.9% 차)로 **이미 정합**이라 손대지 않았다.

### 10.5 이번에 실제로 바꾼 것

```
.blend  SM_ElevCab_Panel_AD57.blend   UVBox 30 loop + UVUnique 6 loop
        23,189,186 → 23,184,899 bytes   백업: <scratchpad>/…pre_uvfix_backup.blend
        UV 레이어 이름·순서·active_render 불변 → AD48_EXPORT 전제조건 그대로

Assets/…/BakedAO/AO_SM_Gauge_Housing.png    512    ← 이 한 장이 증상의 원인
Assets/…/BakedAO/AO_SM_Gauge_Fill.png       256
Assets/…/BakedAD47/BK_SM_Gauge_Housing.png  1024
Assets/…/BakedAD47/BK_SM_Gauge_Fill.png     256
```

**FBX 는 다시 export 하지 않았다.** 지오메트리가 안 바뀌었고, `AD48_EXPORT` 는
슬롯 0 을 `UVBake` 로 바꿔치기해 내보내므로 Unity 가 읽는 TEXCOORD0 에
`UVBox`/`UVUnique` 는 애초에 실리지 않는다.

**안 건드린 것과 그 이유** — 해상도를 함께 올리려다 되돌렸다. 1024 로 구운 판을
같은 해상도로 다시 대조하니 `AO_SM_Gauge_Labels` 는 **maxΔ 0, 비트 단위 동일**이었다.
즉 처음에 보인 53% 차이는 전부 내가 바꾼 해상도 탓이었고, 지오메트리 변경은 라벨
AO 에 **아무 영향이 없다.** 요청받지 않은 회귀가 될 뻔했다.
`BK_SM_Gauge_Labels.png` 도 3화소 잡음뿐이라 넣지 않았다.

> **남은 불일치(의도적으로 안 고침)**: 하우징의 알베도는 1024 인데 AO 는 512 다.
> `ad58_bake_ao.py` 는 「AO 는 알베도와 같은 해상도여야 한다」고 적어 두고 해상도를
> `AD53_BAKEALL` 의 `SIZES` 에서 읽는데, 그 `SIZES` 가 하우징·라벨을 512 로 적고 있어
> **낡았다**(실제 알베도는 1024). 이번엔 증상 수정과 해상도 변경을 섞지 않으려고
> 기존 512 를 유지했다. 512 에서 홈 아일랜드는 9px 인데 `margin=8` 이라 거의 전부
> 블리드다 — 그래서 값이 균일한 **0.0**(std 0.0)으로 나오고, 1024 에서는 22px 라
> 참값 **~10**(std 1.0)이 보인다. **둘 다 끝까지 균일한 검정이라 증상은 어느 쪽이든
> 해결된다.** 올릴 거라면 `SIZES` 부터 고치고 하우징·라벨을 함께 올려라.
