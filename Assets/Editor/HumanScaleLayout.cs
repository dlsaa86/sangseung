...
/// 좌표계: 바닥 윗면 y=0.
///
/// ⚠ **치수는 이 파일이 소유하지 않는다.** `TenFloorSceneBuilder.Reproportion()` 이
/// 나중에 돌면서 껍데기를 덮어쓰므로, 실효 치수는 그쪽 상수다.
/// 이 주석에는 「폭 3.2m(x ±1.6) · 깊이 3.0m · 높이 2.5m」라고 적혀 있었는데
/// **그건 이 파일이 처음 작성될 때의 값이고 그 뒤로 두 번 바뀌었다.**
/// 2026-08-02 실측(렌더러 월드 바운드)에서 실제 씬은 2.40 × 3.00 × 3.20 이었다 —
/// 낡은 주석 둘을 근거로 하루에 세 번 틀린 치수로 계획이 섰다.
///
/// 현재 실효 치수 (`TenFloorSceneBuilder` 의 `InnerHalfWidth`·`InnerHalfDepth`·`InnerHeight`):
///
///     실내 폭 4.80 m (x ±2.40) · 깊이 6.00 m (z ±3.00) · 높이 5.50 m · 바닥 28.80 m²
///
/// 저 셋 중 하나라도 바꾸면 **이 주석과 `DEVICE_DESIGN_SPEC.md` 를 함께 고친다.**
///
