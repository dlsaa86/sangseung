...
namespace Ascend.Prototype.EditorTools
{
    ...
    public static class AscendTestMenu
    {
        ...
        private static (int passed, int failed, string report)[] AllSuites() => new[]
        {
            ...
            // 밸런스 시뮬레이터가 **게임과 같은 규칙으로 도는가** (2026-08-05).
            // 시뮬은 `SpinSeed.Derive` 대신 순차 스트림을 썼고 앤티 식을 손으로 복제하고
            // 있었다. 총량 통계는 우연히 일치했지만 — 둘 다 유효한 PRNG 스트림이므로 —
            // **시드별 개별 런은 98% 어긋났다**(1층 최종 전력 일치율 0.1~2.2%).
            // 즉 「시드 N 에서 이렇게 됐다」는 문장이 게임을 설명하지 않았다.
            // 세 번째 단정이 실제 `FloorSession` 을 시뮬 옆에서 나란히 돌려 대조하므로,
            // 시뮬이 다시 갈라지면 그 순간 여기서 잡힌다.
            Sim.Tests.SimulatorParityTests.RunAll(),
            // 위험 판정이 **프레임이 아니라 상태 전이에 묶여 있는가** (`D-1` 2026-08-05).
            // 평가기는 히스테리시스 때문에 입력 궤적의 함수라, 부르는 자리가 곧 판정이다.
            // 직전 판본은 `RiskStateView.LateUpdate` 한 곳에서만 불렀고 — 과수확 레버는
            // 앤티 지불과 스핀을 **같은 프레임에** 끝내므로 — 그 사이의 봉우리를 관측하는
            // 프레임이 존재하지 않았다. 사고 기록이 시드가 아니라 프레임 타이밍에 달렸다는 뜻이다.
            // 이 스위트가 헤드리스(씬 없음·뷰 없음)에서 도는 것 자체가 소유권 이전의 증거다.
            Run.Tests.RiskDeterminismTests.RunAll(),
        };
