...
namespace Ascend.Prototype.Spin
{
    ...
    public class SpinRuleSet
    {
        ...
        ///
        /// 🔴 **2026-08-09 — 14.0 → 11.5 로 내렸다가, 실측하고 14.0 으로 되돌렸다.**
        /// (`PLAN_BUILD_DEPENDENCY.md` §C-3·C-7 1단. 아래는 그 시도와 되돌린 근거다.)
        ///
        /// 사용자 지시("레버만 당겨서는 게임을 통과할 수 없게")를 좇아 14.0→11.5로
        /// 내리고 `SpinEngine.FindMatches`에 Duo·Cross 판정을 얹었다. 자체 검증에서
        /// 새 실패 3개가 나왔다 — "무계약 6층 도달 0/6"(직전 1/6, **악화**), "적재해도
        /// 10층 완주 0개", 그리고 결정적으로 **시드 271828이 1층에서 즉시 죽었다**
        /// (9층까지 가던 시드가 0층으로).
        ///
        /// ## 왜 — Duo·Cross는 1~5층에서 원천적으로 못 뜬다
        ///
        /// `PrototypeCurriculum`의 1~5층은 `SymbolPool = SoulAndAbsorber` — 저항체가
        /// **흡수체 하나뿐이다.** Duo·Cross는 "다른 저항체와의 관계"로 성립하는
        /// 패턴이라 저항체가 하나뿐이면 정의상 불가능하다(증식체가 들어오는 6층부터만
        /// 가능). 그런데 `NormalSoulValue`는 전 층 공통 필드다 — 내리면 1~5층도
        /// **똑같이** 어두워지는데, 그 층들엔 어두워진 만큼 벌충해 줄 패턴이 없다.
        ///
        /// 콘솔 스크래치로 실측(`PrototypeCurriculum.BuildRules` 그대로, 무빌드,
        /// 계약 없음, 층당 300시드, `docs/runtime/PATTERN_IMPL_NOTES.md` §6 참고):
        ///
        /// ```
        ///                    NSV=11.5           NSV=14.0(원래값)
        ///   1층 크래시율      5.7%                1.0%     ← 271828이 여기서 63%로 죽었다(70% 미만)
        ///   2층 크래시율      5.0%                2.0%
        ///   3층 크래시율      7.7%                5.7%
        ///   4층 크래시율     43.3%               30.0%     ← "4~5층에서 막힌다"는 여기서 이미 자연 발생
        ///   5층 크래시율     56.7%               41.0%
        ///   6~9층(Duo·Cross 켬) 34~59%           34~55%     ← Duo가 스핀당 다발로 터져도(런당 5~7회) 못 메운다
        /// ```
        ///
        /// 4~5층은 **11.5 없이도**(즉 옛 14.0 그대로도) 이미 30~41%/41~57% 크래시율로
        /// "무빌드 런이 막히는" 구간이었다 — 그 압박은 요구 전력 곡선이 이미 만들고
        /// 있었지 이 필드가 만들 필요가 없었다. `NormalSoulValue`를 낮춘 결과는
        /// "패턴이 필수가 됐다"가 아니라 "패턴이 안 통하는 1~5층까지 다 같이
        /// 어려워졌다"였다.
        ///
        /// ## 결론 — 이 필드는 원복하고, "패턴 없이는 못 간다"는 요구 전력 곡선 자체
        /// (6~10층이 970→1670으로 급등하는데 저항 2종을 다뤄야 하는 것)와 Duo·Cross
        /// 판정에 맡긴다. `PatternKind`·`FindMatches`의 판정 로직은 그대로 둔다 —
        /// 문제는 패턴 판정이 아니라 이 필드 하나였다.
        ///
        /// 되돌리려면(즉 다시 낮추려면) 이 값을 내리는 대신, 6층 이상에만 적용되는
        /// 별도 난이도 축(예: 층별 `NormalSoulValue` 오버라이드, 아직 없음)을 새로
        /// 만들어야 한다 — 이 필드 하나로는 1~5층과 6~10층을 갈라 건드릴 수 없다.
        /// </summary>
