...
namespace Ascend.Prototype.Art
{
    ...
    public static class ReferenceRoomSpec
    {
        ...
        /// <summary>
        /// 표시기 판독면 폭(m). **0.72 → 0.90 (2026-08-04, 3차 평가 회신).**
        ///
        /// 폭은 벽이 막는다 — 중심 X 가 레버 컬럼 우측 끝(0.928)에서 유도되므로
        /// 오른쪽 끝 = 0.928 + 폭이고 우벽이 2.00 이라 **상한이 1.07** 이다.
        /// 0.90 이면 오른쪽 끝 1.828 로 0.17 의 여유가 남는다.
        /// </summary>
        public const float PowerMeterWidth = 0.90f;

        /// <summary>
        /// 표시기 판독면 높이(m). **0.50 → 0.92 (2026-08-04, 3차 평가 회신).**
        ///
        /// ## 왜 값을 키웠나 — 이건 값 조정이 아니라 배치 결정이다
        ///
        /// 독립 평가 실측: 계기 여섯 줄의 한글 글리프가 `A_entry_to_machine`(카메라
        /// 거리 4.16m)에서 **6~7px** 였다. 한글은 초·중·종성이 쌓여 안정 판독에
        /// 약 16px 를 요구하므로 그 크기는 판독 불가가 아니라 **텍스처 노이즈**다.
        ///
        /// 배율은 `AscendReferenceRoomRewire.MovePowerReadout` 이
        /// `min(판독면 / 내용 크기)` 로 정한다. 즉 글자를 키우는 방법은 둘뿐이다 —
        /// **판독면을 키우거나 내용을 줄이거나.** 두 가지를 다 했다.
        ///
        ///   내용 폭  1.79 → 0.70 (막대를 글자 단 폭으로 줄이고 과적등을 단 안으로,
        ///            그리고 TMP 의 부푼 `Renderer.bounds` 대신 **글리프 상자**로 잰다)
        ///   판독면   0.72 × 0.50 → 0.90 × 1.05
        ///   배율     0.339 → 1.018  (**3.00배**)
        ///
        /// 2.5배(0.92 높이)에서 멈췄다가 캡처의 **잉크 높이가 13~15px** 로 나와
        /// 평가자가 제시한 한글 안정 판독 하한 16px 에 못 미쳤다. 배율은 2.54배로
        /// 요구를 만족했지만 화소는 만족하지 않았다 — **요구는 화소로 적혀 있었다.**
        /// 그래서 폭까지 벽 여유 안에서 키워 3.0배로 올렸다.
        ///
        /// 세로는 막는 것이 없다 — 1.05 면 y 0.83…1.88 로 선반(0.95, 우벽 평면)·
        /// 장치(x −1.5…0.7)와 겹치지 않는다. 차분 렌더로 가림 0 을 확인했다.
        /// </summary>
        public const float PowerMeterHeight = 1.05f;
