...
.DESCRIPTION
    ...
    원래는 짝이 있었다. `.claude/hooks/unity-preflight.sh` 가 죽은 에디터를 감지해 MCP
    호출을 즉시 차단하는 쪽(감지)이고, 이 스크립트가 되살리는 쪽(복구)이었다.
    **감지 쪽은 2026-08-03 에 지웠다(9723ffb). 지금 남은 것은 복구뿐이다** — 에디터가
    죽으면 MCP 호출은 차단되지 않고 120 초 타임아웃까지 매달린다.
