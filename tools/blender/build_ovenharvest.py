...
PIPE_R = 0.115         # 통관 반지름. 방 폭의 절반보다 작아야 「관」으로 읽힌다
...
def build_cabinet(M, coll):
    ...
    pipe_cy = PIPE_R - CHAMBER_PROUD     # 관 앞면이 방 앞면과 같은 평면에 닿는다
    hh = CHAMBER_H * 0.5
    for cx in COL_X:
    ...
    # ── ⑦ 상단 적색 표시등 — 레퍼런스의 붉은 돔 ─────────────────────────
    lamp_x = W * 0.5 - 0.150
    box(b_trim, (lamp_x, -0.036, sh_c), (0.130, 0.030, 0.130))
    prism(red.bm, (lamp_x, sh_c), 0.052, -0.086, -0.050, 10)
...
def build_column(M, coll):
    ...
    prism_z(t, (ox, rod_y), 0.022, 0.50, z_car + 0.055, 12)      # 막대
    ...
    box(t, (ox, (rod_y - 0.130) * 0.5, z_car), (0.048, rod_y + 0.130, 0.048))
    cyl_x(t, (ox, -0.130, z_car), 0.026, 0.064, 10)
    grip = Part("OH_LeverGrip", M["red"])
    cyl_x(grip.bm, (ox, -0.130, z_car), GRIP_D * 0.5, GRIP_LEN, 14)
    for s_ in (-1, 1):
        cyl_x(grip.bm, (ox + s_ * (GRIP_LEN * 0.5 + 0.011), -0.130, z_car),
              GRIP_D * 0.5 + 0.011, 0.026, 12)
    grip.finish(coll, bevel=0.001)

    # 막대가 돌리는 것 — 아래 플라이휠
    p_low = (ox, COL_BOTTOM + 0.28)
    low_pin = (ox, rod_y, p_low[1] + 0.086)
    strut(t, (ox, rod_y, 0.52), low_pin, 0.030, 0.024)
    cyl_x(t, low_pin, 0.016, 0.058, 10)
    prism(t, p_low, 0.115, -0.082, -0.038, 16)
    prism(t, p_low, 0.126, -0.070, -0.050, 20)
    prism(b, p_low, 0.034, -0.096, -0.028, 8)
    for k in range(6):
        a = TAU * k / 6
        strut(b, (ox + 0.030 * math.cos(a), -0.060, p_low[1] + 0.030 * math.sin(a)),
              (ox + 0.112 * math.cos(a), -0.060, p_low[1] + 0.112 * math.sin(a)),
              0.020, 0.026)
